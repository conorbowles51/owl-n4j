...
| user-bug-8 | bug | Comms Timeline not showing all comms already filtered (only one phone) | Alex | 2026-06-09 20:20 | PARTIALLY FIXED (cross-identity filter) |
...
> - **user-bug-8 PARTIALLY FIXED (round-4)** — ROOT CAUSE: identity fragmentation (one human = many Person nodes per app/device). Built a **cross-identity filter**: `neo4j_service._identity_clusters` auto-links identities that share a strong identifier (phone number / handle / user_id / email; an identity's key appearing as another's user_id), via union-find with a promiscuous-identifier guard (drops generic/service ids; name is NOT used). New `expand_identity_cluster` + an `expand_identities` flag on get_cellebrite_comms_between/threads/envelope (+ routers, api.js). Comms Center has a **"Link identities" toggle** (default **OFF** — opt-in; the per-device/contact-book perspective stays the baseline per [[feedback_cellebrite_perspective_primary]]) wired to the thread list, envelope, cross-type timeline and PDF export. The filter is read-only — it never merges data or changes the per-device names. Verified: 23.5k persons → clusters in 1.8s (cached); max cluster 13 (no over-merge); 3,725 humans unifiable; live demo "Chepe" filter 85→179 items / 2→3 reports.
>   - **STILL OPEN: name-only fragmentation.** The tester's specific "Kathy" identities share NO identifier (phone-12404323444 vs telegram-5960632470 — only the display name, and there are 4+ distinct "Kathy" people), so they're NOT auto-linked (auto name-merge would wrongly fuse different people). Those need a **manual merge** (Contacts → Merge identities); the filter then respects it. Logged as KNOWN LIMIT in the hub (`known-name-only-identities`).
