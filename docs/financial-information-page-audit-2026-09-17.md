...
## Printed charge comparisons and final difficult-file review, 18 September 2026

Proposal v27 recognises a spaced card-payment label even when an unreadable reference precedes it. An amount without its printed minus remains unresolved, with an instruction to check both the date and amount. The date and digits are preserved; the description does not supply a sign. Five additional affected rows were compared with the original wide line crops, and every original has a trailing minus. The previously unflagged date error on physical page 35 is now reached through this row's problem link.

Recognised Merrick fee and interest sections now retain their separately printed totals as controls. The shared server calculation sums only the current included charges in that section, including credit adjustments. It refuses a match for duplicate or unreadable totals, unreadable charges or manually added payments with unknown section placement. Year-to-date figures never become period totals. Differences link to both the total and contributing charges, and checks recalculate after correction.

Across 42 retained Merrick sections, all fee comparisons match zero; 34 interest comparisons match, four differ and four are unavailable. The four differences are all 0.20 reading errors, on physical pages 17, 28, 34 and 45. Every affected original charge and printed total was inspected. No number was changed to force agreement. Statement IDs, original cells, included entries, dates, descriptions, amounts and balances are unchanged. Only the five newly recognised sign problems remove the unsupported debit interpretation. Readiness remains seven ready reviews with 15 payments and 35 reviews needing attention; these checks expose specific errors inside reviews already held for other reasons.

Focused verification: 89 affected backend tests, 45 existing frontend checks, the new arithmetic-component check and five period-selector checks pass. TypeScript and scoped lint pass. A fresh synthetic browser journey corrects the interest amount and payment date/sign beside the original, recovers the saved review after clearing tab storage, imports exactly three payments once and opens the original source from Transactions. No JavaScript errors; both light-mode captures were inspected. Two initial browser-script selectors were corrected to follow the already open inline editor and its actual date label. No real case was modified and no successful import was repeated.

Private records: `merrick-new-controls-summary.json`, three inspected `merrick-new-controls-*.png` sheets, `merrick-v27-readiness.json` and `financial-charge-browser.json`. These checks close the concrete defects found by the independent sample. They do not establish perfect OCR or a general accuracy percentage. Difficult scans still require correction of damaged fields, two Merrick names remain unreadable, and genuinely unassigned payment/history pages require an investigator's decision. The known non-payment false candidates and wrapped-payment omissions found in this benchmark are resolved, with their original sources retained. No further specific unresolved classification or workflow defect remains in this audit. Deployed verification and team acceptance remain separate.
