...
4. Search a large batch by a currency token in filenames. Select all matching statements across pages, change the search and check the hidden-selection count. Choose a currency and apply it; the list must include GBP, CHF, JPY and KWD as well as USD, MXN and EUR. Reopen a selected statement: currency, manual account edits and corrected amounts must persist. Unselected and imported statements must remain unchanged. This is not currency conversion. In a synthetic test statement, change between two-decimal, yen and three-decimal currencies: printed numbers must stay the same after reopening. A fractional amount that yen cannot represent must refuse the change without changing any selected statement.
...
10. For an eligible old BBVA import containing only empty incomplete records, use **Save [number] payments to Transactions**. Check its preview against the PDF first. Expect the recognised payments and balances, no remaining empty records, and the updated batch count. Retry after a simulated lost response: no duplicate payments. The old import must remain in history. Saved payment corrections must prevent this shortcut from displacing them. Saved account details and page-cited balance edits must survive recovery.
...
13. For a PDF spanning a Capital One product-heading change from World Elite Mastercard to Quicksilver, confirm every printed period appears. On a continued transaction page, check both dates and the amount against the PDF. A conflicting account or period must remain unresolved. In an older batch, select **Check for additional statement periods**: existing imports and saved corrections must remain while newly recognised periods become available. Refreshing and retrying must not duplicate payments.

Local automated checks do not replace these tests against the deployed version and the team's actual saved imports.

### Direct review-to-payments acceptance

- In a batch, open one statement, correct the holder, and select **Import [number] payments and view Transactions**. Expect the exact statement's payments immediately; other ready statements remain unimported. Reopen and reload: the holder stays saved and payments are not duplicated.
- Import a statement containing only balances from that same review. Expect a saved-balances confirmation with its account and balances still visible, not an empty Transactions view.
- Recover an eligible older empty import. Compare the displayed usable payment count with the resulting ledger, and verify saved account/balance edits survive. Incomplete readings must not be described as payments.
- With multiple incomplete statements and more than one page of retained records, expand the statement list. Each affected PDF must be available, including files whose records are beyond the first page.
