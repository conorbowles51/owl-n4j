...
Shared ledger reads now suggest merchant/payment-type categories and named counterparties from descriptions, with per-field origin, rule version and explanation. Suggestions reach Transactions, category filters, Trends, name profiles and exports without rewriting source readings, amounts, currencies or balances. Card payments remain debt payments, merchant credits remain distinct from income, and transfer intermediary banks/account references are not invented as final beneficiaries. Recorded counterparty fields and investigator edits take precedence. Explicit rejection/clearing persists; confirming a suggestion records its former value and origin in edit history. Names/categories in exported reports are marked as suggested. Recognised card entries with no printed row balance say Not printed; unestablished gaps say Not available, with source explanations.

Replaying the actual 672 descriptions yields 588 category suggestions and 446 counterparty suggestions. Remaining descriptions lack sufficiently clear cues for these rules. This is reproducible rule-based assistance, not a claim of complete classification across the 588-file corpus. Validation: 106 backend tests and 46 frontend tests pass, plus scoped ESLint and a production build (existing chunk-size warning). Authenticated post-deployment verification of the live row identities, amounts, currencies, balances and UI follows publication. Private audit data remains ignored under data/local-runtime.
...
The current reading can now be explicitly saved to Transactions, preserving saved holder/account details and page-cited balances, retaining the prior import in history, and refusing to displace payment corrections. The displayed count includes usable payments only. Identical saved rows can adopt a metadata-only revision without another review. A batch statement review also has a direct import-and-view action backed by the existing durable batch job; draft saving remains separate. Balance-only batch imports stay beside their saved account and balances with a confirmation. Incomplete-record links cover every affected statement, not only the first page. Explicit import navigation clears old category filters.

The supplied six-page PDF was extracted with production geometry and replayed in an isolated database, including the older empty import, saved account/balance edits, identical old review, recovery, current batch receipt, reopening and repeated confirmation. All 17 payments appear: six credits totaling USD 182,916.33 and eleven debits totaling USD 217,063.72; opening USD 191,772.17 reconciles to closing USD 157,624.78. The simulated native extraction has 249 incomplete rows; the observed live legacy import has 245. These are different extraction versions. The live 204-row saved review was separately compared and every normalized row matches the current reading.

Verification: 84 focused backend tests, 69 component tests and five Chromium journeys pass, including the direct batch route permission/revision check and balance-only destination. Production build and scoped lint pass. Authenticated live recovery verification follows deployment; do not treat the local replay as proof of a repaired live case. Private PDFs, snapshots and audits remain in ignored local runtime data. The full 588-file corpus remains unverified.

## 20 September Transactions opens all accounts, with visible holder filters

The preceding multi-statement fix is live as `f855b713` (Cheeky Strudel), verified through public version, compiled controls, guides and the protected refresh route. Alex's next feedback was that Transactions appeared to require choosing an account before loading. The ledger already reads the whole case by default, but saved account/date scopes and explicit statement/batch navigation could keep its displayed result narrow.

Selecting the main Transactions tab now resets those filters and opens all current imported payments automatically. A fresh unqualified visit to a remembered Transactions page also starts with the whole case. Explicit statement/batch/account links keep their intended scope, and browser refresh preserves the chosen view. A visible **Show all imported payments** action clears account, dates, category, statement/batch and table filters together without changing records, findings or selected payments.

**Person or company** and **Bank account** selectors are visible above the payment table and update immediately. A holder includes matching recorded holder names across banks, normalised only for case and whitespace; accounts and entities are not merged. Options come from the complete loaded transaction set, without the account-search endpoint's result cap. Totals and 50-row pagination use the same selection, and table downloads record and apply the holder/account filters. Date changes retain these table choices. The date range remains collapsed until needed; the redundant account selector is removed from that control in Transactions.

Verification: 20 backend tests, 109 component/API/navigation tests and one Chromium walkthrough pass. The screenshot was visually inspected. The real components exercise 123 synthetic payments across three accounts/two holders, mixed currencies, account narrowing, paging, date/filter retention and return to the whole case. The unit/export regression uses 125 rows; backend export checks confirm equivalent filtering without altering the full captured ledger. TypeScript, production build (existing chunk-size warning), scoped lint and diff checks pass. The guides and downloadable checklist reflect the new navigation contract. No live case data was changed; public publication verification follows the authorised push.

## 20 September supplied multi-statement Capital One file verified

The preceding Alex workflow release `7efb1b91` (Frosty Yeti) was pushed and publicly verified live. Testing the newly supplied 201-page Capital One PDF exposed two further layout gaps: only five of its 51 printed periods were detected because the product heading changes to Quicksilver; a continued transaction page omitted the repeated issuer/holder heading, leaving 11 amounts unresolved. The reader now recognises that exact product heading and binds the continuation to its printed account, cycle, columns and matching holder total. Source citations and saved revision binding are retained.

The actual PDF now yields 51 periods, one USD account and 653 transactions and charges, with zero incomplete payment records and all 51 opening/closing checks matching. The date coverage is 30 November 2020 through 20 February 2025. A fresh production table extraction fed an isolated database journey: reproduce and import the original five periods, save a correction, refresh the existing batch, import the additional 46, reopen every period, inspect all three pages of statement checks, query the full batch transaction scope, then repeat confirmation and refresh without duplicates. The saved correction survives. The complete journey passed in 223.9 seconds; this is a local test duration, not a production ETA. Private PDF data and the detailed audit remain ignored under `data/local-runtime/capital-9392-inspection/`.

Existing batches offer **Check for additional statement periods**. This reuses saved extraction and preserves edited/imported/excluded items; it does not reupload or automatically import. The case-scoped route requires edit permission. **Open imported transactions** now follows verified replacement readings, matching the current batch receipt after a BBVA recovery. The guides and downloadable team checklist describe the refresh action.

Validation: 129 focused backend tests and 10 batch-panel component tests pass, along with TypeScript, the production build, scoped ESLint and diff checks. The build retains its existing chunk-size warning. The actual-file journey covers import/reopen and retry; the new button is covered by component tests, not an authenticated live browser journey. No live case data was changed. Full 588-file and unavailable ARRENDO acceptance remains open, as recorded in `docs/alex-financial-acceptance.md`. Publication verification will be recorded separately after the authorised push.

## 20 September complete Alex workflow repair pass

This supersedes the earlier checkpoints below. Release `86a32cde` (Mighty Taco) was deployed before this pass. The new changes address saved reviews and imports as well as new extraction: optional notes for post-import corrections, a persistent Mark checked action, safe reader-version upgrades for unchanged saved drafts, and current batch receipts after replacement/correction. USD/MXN/EUR can be corrected after import in the inline account/balance editor without converting amounts; earlier readings, categories and exclusions remain. Choosing a missing currency completes every otherwise-usable retained record atomically. BBVA printed Spanish currency names and the older holder address block are recognised within their layout context.

An eligible old BBVA import containing only empty incomplete records now offers Update saved reading and open results using its existing PDF. This is explicit, retry-safe and retains the earlier reading; it refuses to displace saved payments or row/balance corrections. A supplied-PDF replay recovered 223 simulated old incomplete records to two payments, zero incomplete records and a matching 60.00 → 25.20 balance with 34.80 charged. This is not an authenticated repair of Alex's existing import, whose screenshot showed a different legacy extraction count.

Alex's additional feedback is included: case-wide checks moved to Review accounts → Account checks and are absent during statement review. Repeated batches show their starter, date, distinct identifier, sample files and reading progress. No testing or data repair has been performed in the client case; Alex's chosen acceptance case is Neil finance.

Verification: 187 backend tests, 117 component tests and the additional Mark checked test pass. Two Chromium browser tests pass with both current light-mode screenshots inspected. The actual supplied PDF passed import, save/reopen of account/balances, categories, EUR/MXN/USD currency roundtrip, payment visibility, working totals, statement checks, retained history and idempotent retry in an isolated database; a separate actual-source journey passed legacy-import recovery. TypeScript and the production build pass (existing chunk-size warning), scoped ESLint and diff checks pass. Guides and the downloadable test pack are updated. Full acceptance remains open for the unavailable ARRENDO PDF and full 588-file corpus; their location was requested. See `docs/alex-financial-acceptance.md`. Publication verification will be recorded separately after the authorised push.

## 20 September account and balance corrections, batch currencies and review dead ends fixed locally

Alex's follow-up showed an actionable UI defect: a statement with no recognised payments or balances required a balance, disabled saving and sent **Show items to check** to another explanation with no input. The review now has labelled opening/closing fields, including an **Add opening balance / Add closing balance** action when extraction supplied no control, with a PDF page for each manual balance. The check jumps to these controls. Zero stays distinct from missing, notes are optional and **Save statement balances** replaces the misleading save-zero-records wording. In a batch this saves for later confirmation; individual review saves the statement directly. Missing holder/number checks lead to those fields. Saving a holder correction clears its batch warning.

Imported statements now have **Edit account and balances** beside the original PDF, including from a statement check. Saved account numbers and leading zeros survive reopening. Changes keep payment IDs and source references, retain actor/time and earlier values, reject stale edits and update balance checks. A changed account identity moves only this source's period/payments; existing shared account labels are not globally overwritten. Original sealed import records remain unchanged, with later page-cited balances overlaid for checks. Earlier corrections remain visible when rereading. File lists and imported review distinguish usable payments from incomplete records, including an all-incomplete import.

The batch offers searchable statement selection across all pages and explicit USD/MXN/EUR assignment. Selection can include multiple periods of one PDF, hidden selections are counted, saved manual edits survive and the operation rolls back atomically if a selected period changed or is unavailable. Already imported statements cannot be relabelled through this action; they require a corrected replacement import. Generic MXN amount reading accepts a peso symbol after MXN is explicitly chosen; it does not infer a currency from an ambiguous dollar sign or filename.

Proposal v29 keeps text without recognised transaction columns as unclassified source text, excluded by default, rather than creating empty selected payments. It remains inspectable and editable beside the PDF. Damaged readings inside recognised transaction tables remain payment checks. Batch counts separate usable transactions, incomplete records and unclassified text. Large issue lists collapse per statement. Existing unreviewed batch items use the current assessment; saved drafts from an older reading require comparison, and existing imported records are not silently rewritten. This does not establish complete extraction of all 588 PDFs. The BBVA Cash Management recogniser also accepts the older BBVA Bancomer bank heading; this is covered with synthetic geometry. The actual ARRENDO January 2021 PDF was not provided locally, so support for that specific file is not claimed from its screenshot.

Validation: targeted backend regressions passed for imported-detail edits, manual balances, source citations, duplicate/retry behavior, permissions, account/holder persistence, bulk currency preservation and rollback, BBVA including old-import replacement, Capital One, Merrick, Andrews, generic proposals, review progress, arithmetic and recovery. The latest frontend run passed 64 tests across six affected components. Two Chromium UI tests exercised the pre-import balance path and post-import saved-details path; their synthetic source placeholders test interaction/layout, not PDF extraction. Both light-mode screenshots were inspected. TypeScript, scoped ESLint, production build and diff checks passed; the build retains its existing large-chunk warning. Guides and acceptance checklist include the new controls. No authenticated test against Alex's live batch, live repair, push or deployment has been performed for this checkpoint. Last repository commit remains `05a0e543`.

## 20 September BBVA Cash Management reading fixed locally

The supplied seven-page BBVA Mexico euro statement contains two genuine charges below its balance summary: a 30.00 account fee and 4.80 VAT. Its opening balance is 60.00 and closing balance 25.20. The earlier generic parser did not recognise the Spanish transaction headings and treated non-payment text as incomplete transaction candidates. A fresh native-table replay reproduced that failure mode with 223 candidates; the screenshot's earlier saved extraction reported 250, so this is not claimed as an exact replay of the production extraction.

A dedicated recogniser now binds BBVA Cash Management pages to their printed account, period and page sequence. It reads labelled currency (including MONEDA EUROS), Cargos/Abonos columns, operation and liquidation dates, wrapped descriptions and following-page continuations. It keeps operational balances together instead of mixing them with liquidation balances. Summaries, charts, disclosures, glossary and fiscal text remain source rows, outside transactions. Printed counts and amount totals provide separate checks. The same layout with no movements saves balances without inventing payments. This is scoped format support; other BBVA products and the unprovided ARRENDO file are not established as supported.

Verification uses the actual supplied PDF through the production native-table reader, source validation and import writer in an isolated test database: EUR, one account/period, two transactions, no incomplete records, matching opening/closing and direction totals, preserved date roles and source locations, and idempotent retry. All seven pages were visually inspected; the first is an image-only informational cover. The private audit and extracted material remain ignored under `data/local-runtime/bbva-inspection/`. Twelve synthetic BBVA regressions cover zero-payment statements, damaged/missing amounts, counts, separate periods/accounts, distinct balance conventions and replacement of a previously incomplete-only import while preserving its history. Across targeted runs, 66 backend tests and 41 frontend tests pass. One existing batch test still expected incomplete imports to be blocked; its failure was reproduced with the baseline issue code, then the assertion was updated to verify the current nonblocking contract and retained missing value. TypeScript, scoped ESLint, production frontend build and diff checks pass; the build retains its existing large-chunk warning. No full application suite or authenticated deployed verification was run.

The incomplete-record area now explains why its rows cannot contribute to totals and offers a visible return to the source statement. An all-incomplete import no longer claims to be showing usable payments. Guides and the test pack include BBVA and replacement checks. Existing live imports are not silently changed: reread the file with PDF text and explicitly replace the previous import. This checkpoint is prepared for the user's requested push; deployed version and public assets will be checked after publication. Authenticated reprocessing of the existing live case remains a separate acceptance check.

## 20 September direct bulk preparation and import actions implemented locally

The financial file register now supports selecting PDFs and preparing all their recognised accounts/periods as one batch, without reopening the Evidence picker or individual reviews. Select all applies to the shown files; filtering retains selected files and explicitly counts hidden selections. Failed preparation retains the selection and reuses its request ID. The batch opens directly and existing imports remain recognised by the existing backend. Preparing is separate from confirming import. Balance-only batches offer Save statements instead of Import 0 records.

Individual reviews now expose Import statement now above the PDF, sharing the existing confirmation guards and request with the footer. The summary counts selected records with missing/invalid fields and explains that they can be corrected after import outside totals. The instruction to check each flag before import is removed. Bulk blank-row exclusion is now labelled Exclude [number] blank rows from import, with Undo excluding blank rows. The earlier routine-note, labelled-field and balance-only work below remains included.

Validation: 43 individual-review unit tests, 15 file-list/batch unit tests, five Chromium workflows and seven backend optional-review tests pass. Browser coverage includes a 588-file selection retained through filtering and sent in one preparation request, importing 43 records (three incomplete) directly without opening corrections, 1,200-row correction paging and cross-page blank-row exclusion/undo. The first direct-import browser assertion was corrected to check hidden correction inputs rather than absence from the DOM. TypeScript, scoped ESLint and diff checks pass. Light-mode control screenshots were inspected using synthetic fixtures; they are not real-PDF extraction proof. Guides and downloadable checklist are regenerated.

Read-only replay of the supplied 222-page Capital One extraction through the current layout reader and import assessment finds 52 periods and 301 records available without edits: 51 periods have matching balance checks, none have a balance difference and one has a retained reading issue. Original source cells are reused in an isolated test database; no production import or fresh OCR was run. The initial replay used the historical cached layout context and was rerun with the current source-context calculation before retaining the receipt at ignored `data/local-runtime/capital-inspection/audit/bulk-readiness-20260920.json`. This is not an independent extraction accuracy or end-to-end user-time measurement, nor processing 588 real PDFs. No live case data was changed. The production frontend build also passed for the requested release, with the existing large-chunk warning. The shared branch was fetched and had no incoming commits. This checkpoint is prepared for the user-requested push; live verification follows publication.

## 20 September statement review friction fixes implemented locally

Alex's acceptance feedback identified empty entries needing individual exclusion, mandatory reasons for routine pre-import edits, missing correction labels while scrolling, and balance-only statements being refused. The review now offers one reversible action across every correction page to set aside wholly empty unresolved entries. Partial readings, manually entered payments and rows the reviewer has begun completing stay selected. Excluded source text remains available. Date, description, credit, debit and balance inputs carry visible labels; unmatched rows show their original extracted text.

Routine pre-import field/detail corrections and exclusions no longer require typed reasons in individual or batch import. Optional notes remain. The original proposal, complete reviewed values, author and time are retained by the existing import history. Replacing an existing import and moving payments between accounts still use their explicit decisions. Saved drafts on unaffected readings keep their revisions.

Statements with a readable printed opening or closing balance can be saved with no transactions, including zero or differing balances. Missing controls remain unknown; differences and deliberately omitted possible payments stay recorded. Explicit English and Spanish balance labels are recognised outside transaction tables, with source text retained and narrowly defined comma-decimal amounts supported. Available credit, minimum payments and ambiguous multi-value summaries are not treated as balances. Newly recognised balance summaries get a changed review revision for comparison with earlier saved corrections. The file list recognises saved periods even when their payment count is zero.

Validation: affected backend proposal, import and batch checks pass, including zero-payment persistence, retry without duplication, source retention, corrected details without reasons, differing balances and file status after an isolated batch import. Updated-contract assertions were corrected; the batch file-status fixture was supplied with its missing workspace tables. Focused frontend unit checks pass. Three Chromium checks pass for labels after scrolling, a 1,200-payment review with a late correction, and cross-page empty-entry exclusion/undo preserving partial readings and edits. The latter two were moved from jsdom to Chromium after jsdom's label traversal timed out; the native-browser workflows complete in under a second each. TypeScript and changed-file ESLint pass. Guides and the downloadable checklist are regenerated. No full application suite ran and no live case data was changed.

This checkpoint is local, not deployed. Alex's actual `01 ARRENDO BBVA EUR ENE 2021.pdf` was not available in the local files searched; balance-only regressions use explicit synthetic fixtures. Its real layout and the other statement types still need acceptance on the deployed build. The prior live currency release is `a0b9dd60` (Wonky Muffin).

## 19 September statement currency detection corrected locally

The evening screenshots showed readable dollar values with empty amount fields under an EUR review. Currency inference previously ran only in bulk preparation, while individual review relied on a labelled header or a saved file-wide choice. Individual and bulk review now share detection scoped to each account/period. Labelled codes, monetary cells and recognised issuer layouts establish the currency; ambiguous dollar/yen symbols and conflicting evidence still require a choice. Bulk preparation retains each period's currency instead of overwriting it with a file-wide value. Moving between statement periods clears the prior manual navigation choice. Existing explicit choices remain visible, with a detected-currency action to reread amounts without reuploading. Original imports and revision-bound corrections are retained. The amount warning now identifies the printed value and currency conflict instead of asking generically to check a readable amount.

Read-only replay of the supplied 222-page Capital One extraction detects USD for all 52 periods and reproduces all 301 proposed payments from the explicit-USD baseline. Physical page 3 correctly reads the 246.92 credit and 28.78/21.19 debits without row flags. This is a regression replay, not an independent transcription or production import. The private receipt is ignored at `data/local-runtime/capital-inspection/audit/automatic-currency-20260919.json`.

Validation: 38 distinct focused backend checks pass, including automatic import/retry, mixed USD/CAD periods in one batch, source retention and statement moves. Two targeted frontend workflows pass for correcting a saved EUR choice and retaining edits while switching periods. TypeScript, scoped ESLint and diff checks pass. One new synthetic geometry fixture initially reused an overlapping rectangle; its locator was corrected and its failing test rerun successfully. Guides and the downloadable checklist are regenerated. No full suite ran. This fix is prepared for the user's requested push; live deployment verification remains separate from these local checks.

The preceding investigator-workflow release was deployed as `8a70a540` (Toasty Narwhal). Public verification confirmed that live compiled build, the new protected backend route and exact guide assets; no live case data was changed. Its historical implementation checkpoint follows.

## 19 September simpler investigator workflow implemented locally

The approved design is now connected in the application. Statement import keeps availability separate from reading issues. Complete readings enter the numeric ledger; records missing usable values are retained with their original source, outside totals, and have a later correction action. The correction is atomic, records its author/reason, recalculates the available statement checks and supports retry without duplication. Later reprocessing exposes these corrections for comparison without altering the immutable import request. Batch counts and issue lists reflect completed corrections.

Transactions now use Search and a collapsed Filters control, one selection action bar, directly editable categories, clickable From/To profiles and source inspection beside the list. Related payments has an explicit before/after interval and selected comparison. Findings starts with title and explanation and attaches the selected sources automatically. People, Follow money and chart payment lists share explicit selection, categorisation, comparison and findings; narrowing a view does not silently drop selected payments. Trends has Over time / By category views with the same currency/account-type control and an explicit undated-payment list. Financial tabs participate in browser Back; the existing retained tabs and drafts keep investigation context.

Validation: 76 focused backend tests passed, covering individual/batch import, access checks, missing values, exact retries and review recovery. Across the targeted frontend runs, all 91 distinct unit checks passed after updating the changed-contract expectations; the Chromium investigation walkthrough also passed. The browser task opens an original, selects an outgoing payment, checks its two-day interval and EUR 5,000 difference, saves a finding with both source IDs and returns to the same search. Production build, TypeScript and changed-file lint checks passed. No full application suite was run. Backend tests use an isolated test database; the Chromium task uses real components with synthetic API fixtures and an explicitly illustrative source image.

The user guide, testing checklist, downloadable pack and light-mode screenshots are updated. This is local implementation, not publication or deployed verification. A second authorised session, real PostgreSQL concurrency, and the team's actual multi-period documents still require acceptance on the deployed build. The earlier generic production batch exception was not independently reproduced in this work; these tests do not establish that the user's previously failed production import now succeeds.

## 19 September investigator flow design

The user requested a deep design pass on simpler usability, explicitly for investigators. [The concrete design](financial-investigator-flow-design-2026-09-19.md) builds on the existing accepted workspace and official-product research. Its acceptance task is a receipt → original → related outgoing payment → comparison → finding, with source links and context retained. Import is independent of outstanding checks; incomplete records remain usable with honest totals. The design specifies each existing financial page's purpose, state preservation, phased implementation and acceptance tasks.

A separate clickable, synthetic design sample accompanies the conversation. It demonstrates importing with an unresolved amount, transaction/source inspection, paginated selection, categorisation, comparisons, local findings and monthly/category views. It is not application implementation or deployed verification. No case data was changed and no application tests were run for this design-only work. The existing non-blocking import and financial batch failure gaps remain outstanding; no completeness claim or deployment was made.
...
89 affected backend cases, 45 existing frontend cases, one new arithmetic-component case and five period-selector cases pass. TypeScript and scoped lint pass. A fresh synthetic browser journey corrects interest and payment date/sign, restores saved values without tab storage, imports exactly three payments once and opens the original PDF. Both light-mode captures were inspected; no JavaScript errors. Record `financial-charge-browser.json` is marked imported; do not repeat its import. Initial harness failures were the already open editor and exact date label, fixed without repeating a successful import.

Read-only Merrick readiness remains 7 ready/35 attention, 15 ready payments of 224. All 42 fee comparisons match; interest comparisons are 34 matches, four differences and four unavailable. Private `merrick-new-controls-summary.json`, three original crop sheets and `merrick-v27-readiness.json` retain the evidence. The audit records closure of the concrete blind-sample defects and continuing scan limitations. Both guides were updated/generated with the original 39 light-mode images.

**Final local acceptance:** production build passed (`financial-v27-release-build.log`) and the compiled browser passed corrected transactions, original PDF navigation and both updated guides (`financial-charge-compiled.json`). No imports repeated. D's two remaining classification/review checkboxes are now closed against the recorded original-page audit, with genuine scan limitations stated. Do not rerun successful full suites or 87 disposable imports. Next: finish remote inspection and the final release, then verify the deployed workflow when the current server URL is available. Deployed verification still needs the current URL; do not use the unrelated old IP. Alex's no-intermediate-deployment instruction remains active; remote is still `23039a67`.

Local API 58002 runs v27, session 91179, log `financial-charge-api.log`. Heartbeat arrivals did not start duplicate work.

## Account-name readiness fixes and independent sample, 18 September 2026

After `e1dd325c`, proposal v26 reads complete Merrick coupon names with measured recipient/bank address separation and retains exact name source cells. Forty of 42 original names match visual inspection; two damaged readings remain blank. All 42 statement IDs and payment proposals are unchanged. Andrews accepts a spaced mailing code or doubled closing bracket only with the complete following name/street/city block. Six previously blank account-section names now match the inspected originals, with all IDs/payments unchanged. Missing correction reasons and original row issues now link directly to their payment/page in the batch.

64 focused reader/catalog tests, 51 batch tests and eight facade checks pass. Both guides were updated/generated. Actual imports and immediate retries in disposable databases passed for all 87 ready reviews, creating 339 payments exactly once; no real case or PDF changed. Ready/attention counts: Capital One 51/1 (294 ready payments); Merrick 7/35 (15); large Andrews 22/23 (2); small Andrews 7/9 (28). Balance-only reviews are included in ready counts. See the audit's table and ignored `completion-disposable-imports.json`. Do not repeat this successful import audit without a relevant change.

**New concrete failures from the independent unflagged-row sample:** 18/18 Andrews and 10/12 Merrick rows matched original month/day, amount, direction and running balance. Merrick physical page 17 row 58 reads interest as 36.52 while the original and separately printed interest total are 36.32. Physical page 35 row 42 reads 06/04 instead of 06/01 and loses the trailing minus. Its damaged reference remains at the start of the description, so the existing anchored, unspaced PAYMENT-THANK YOU check fails to notice the sign. Both periods are already held by other issues, but those specific rows must be flagged. Private `unflagged-visual-expected.json` was recorded from five blind original sheets before proposal comparison; the full-width sample-9 crop confirms the minus. No complete-description/year/whole-file accuracy claim.

**Next:** fix card-payment label recognition across spacing and damaged reference prefixes; add independently printed Merrick interest/fee comparisons using current edited values and source links. Preserve genuine mismatches and never infer a field from the matching total. D remains open for these findings. This is useful new work, not another full-suite or unchanged OCR run.

Local API restarted with the holder changes at 58002, session 81035, log `financial-holder-api.log`. Compiled UI remains the successful `fe83d8f` build; documentation generation is newer. No push; remote remains `23039a67`. Alex's deployment hold and the ten-minute continuation are active. A heartbeat arrived during active work and did not start duplicate commands.

## Combined financial regression and compiled acceptance, 18 September 2026

At the completion checkpoint after local `fe83d8fa`, the production build succeeded and the compiled browser passed the 36-payment batch/source-filter/download check, two-payment account-reference/source/finding recovery check, corrected-year transactions, updated guide modal and wrapped-value import/source checks. No browser JavaScript errors. Private records: `financial-completion-scope-browser.json`, `financial-completion-investigation-browser.json` and `financial-wrapped-compiled.log`. No imports were repeated.

One combined regression run covered 4,712 backend unittest cases and 1,386 frontend financial cases. The initial backend run used the older `.venv`, exposing absent libraries; affected 203 cases were rerun successfully in `data/local-runtime/backend-venv`, along with 41 pytest-only cases and 12 real-PDF checks skipped by the older environment. Thirteen missing facade exports and one stale undated-charge assertion were fixed. All eight facade checks pass. Frontend failures were nine stale expectations/fixtures across three modules; all 27 cases in those modules pass after updating the accepted Overview landing, finding-index isolation and current graph count text. These were fixture changes, not disabled checks. Logs use the `financial-completion-*` prefix under ignored `data/local-runtime/`.

A subsequent read-only readiness audit exposed missed printed account names in the difficult collections. That follow-up is being fixed and checked separately; the successful combined regression does not by itself close the difficult-file acceptance work. Continue focused checks for those changes, not another full suite. No push. Alex's deployment hold and the ten-minute continuation remain active; last pushed is `23039a67`.

## Wrapped credit-voucher amounts verified, 18 September 2026

Local only after `d7ed930d`. Proposal v25 reads the two measured Andrews credit-voucher layouts: amount appended to the exact description, or amount and balance immediately beneath it. It preserves every original cell, adds explicit field source references, opens the parent editor when the second-line value is selected, and includes that money line in the imported source highlight. Other layouts or incomplete/conflicting money remain flagged; no number is inferred from balances.

The larger real-file comparison retains 44 ordinary sections plus one assignment-only review and 2,075 entries. Eleven recovered amounts and balances all match the inspected original crops. Flags fall 113 to 102. Eight continuation parent links and nine restored balance diagnostics are the only other changes. The smaller collection's full proposals remain equal: 16 sections, 383 entries, 47 flags. No real source/case writes. Private `wrapped-vouchers-*` comparison JSON/crops in `data/local-runtime/context-audit-final/`.

28 reader tests, one database import test and 43 affected frontend checks pass, with the new test's incorrect accessibility selector fixed and rerun. TypeScript and scoped lint pass after removal of unsupported test-query options. A new synthetic browser case saves and recovers a correction without tab storage, imports exactly three payments, and shows the original PDF across both payment lines. Both light-mode captures were inspected; no JavaScript errors. Browser harness corrections addressed endpoint/navigation/visible-panel selectors without repeating the import. Record `financial-wrapped-browser.json` is marked imported and guards against duplicate writes.

API 58002 restarted locally, session 43862, log `financial-wrapped-api.log`. Both guides updated/generated, including wrapped-value testing. No push; last pushed remains `23039a67`. The ten-minute continuation and Alex's deployment hold remain active. Next: one combined financial regression/build and compiled-browser acceptance at this completion checkpoint, then reconcile the D checklist against the retained original-page audit. Do not claim deployed verification; the current deployment URL question is still pending and the old IP is unrelated.

## Unassigned continuation payments are now reviewable, 18 September 2026

Local only, after `d6e64aef`. A recognised Andrews page with a printed main account/period but no confirmed share now opens as **Unassigned payments**. It cannot import directly. Existing previewed row assignment moves selected payments to a recognised account/period, keeps original locations and corrections, and recalculates both reviews. The emptied review becomes **Payments assigned**, outside ready imports and skip actions. Its empty correction controls disappear. Unknown destinations explain which missing record is needed. Proposal revision is v24.

The larger real collection retains all 44 prior section objects and complete proposals exactly. Ten previously unassigned payments on physical page 13 are additionally available without retyping. No real case or PDF changed. The primary-member application and dividend-only summary on physical pages 1/41 now count as information. Physical pages 3/4 remain unclear forms, 13 remains the unassigned payment source and 42 remains a genuine deposit receipt. Private comparisons: `andrews-large-orphan-comparison.json` and `andrews-large-information-comparison.json`.

Validation: nine database assignment tests, 43 affected reader/catalog tests with one overly broad mixed-table assertion corrected and its 11-test module rerun successfully, 53 focused frontend tests, TypeScript and scoped lint pass. A synthetic browser journey saves the account move, reloads it, imports exactly two destination payments and opens the moved payment's original PDF with no JavaScript errors. The first artificial PDF had a wrong page height; a fresh fixture with matching geometry repeated that journey and its light-mode source highlight was inspected. Record `financial-unassigned-geometry-browser.json`; never rerun its import writer. Earlier browser harness selectors were corrected for combined status text and the compact description button, without repeating completed imports.

API 58002 restarted locally, session 54827, log `financial-unassigned-api.log`; compiled preview still predates these changes. Both guides contain the complete assignment steps and testing cases. No push; Alex's deployment hold and the ten-minute continuation remain active.

The 113 remaining flagged rows in the larger collection were visually inspected across twelve private sheets. They are payment lines, not summary/advertising rows. This exposed another avoidable layout problem: long credit-voucher descriptions can share a cell with their amount or push amount/balance onto the next line. Their values are readable but not currently parsed. **Next: fix this layout with exact source binding, then compare every changed row before closing D.** Do not label the full task complete yet.
...
A six-page read-only comparison recovers four closing-date readings; two other pages use the already readable closing-date label. All 42 sections and 230 entry IDs remain, existing proposed fields are unchanged, and 35 previously missing full transaction dates are completed. Their crops were inspected: 34 match, while page 46's printed 09/25 had already been read as the plausible but incorrect 08/25. An added interest-date/closing-date comparison now flags that problem rather than replacing its month. It also exposes a second such difference on page 26, which requires its own original comparison. Final flags fall 101 to 70, including both new date-difference warnings. No original PDF or case data was changed.

48 focused date OCR tests and 22 Merrick proposal tests pass. Guides/testing steps updated. Private records: `merrick-repeated-date-*`, `merrick-heading-*`, and `merrick-completed-year-crops-*` in ignored `context-audit-final/`. All diagnostic commands are finished. No push; Alex's deployment hold remains. D stays open. Next useful step: recognise the harmless cover/forms/interest-calculation/legal pages in this Merrick pack, while leaving the actual payment-history pages 51/52 available for financial review. The comparison's 14 unclassified pages were inspected as extracted text and include real payments, so do not discard all 14 as notices.

## Cleaned Andrews money lines verified, 17 September 2026

After local `1e006ae1`, measured money cells first use normal single-line OCR on six cleaned crops: two resolutions, three ink thresholds. The primary must be complete, at least four readings must agree across both sizes, and no valid result may conflict. Partial results can use the earlier fallback profiles only when all complete readings agree. Ordinary withdrawal signs are still required from the image; nothing derives money from a balance. Original words and every accepted observation remain in provenance. Readable original cells are untouched. Existing 40-cell/30-second page bounds remain.

This replaces raw-line mode as the first check on these small money cells. A 78-cell experiment found that raw-line mode often lost a minus or added digits; all 65 accepted single-line alternatives matched the inspected original crops. The production whole-file run took 208.76 seconds, retained 16 sections and all 383 entry IDs, and added 77 accepted money refinements: 62 amounts and 15 balances. All 77 crops were visually checked. Two previously accepted amount refinements now have conflicting readings and remain unresolved, rather than keeping their prior values. Existing dates/descriptions and every other previously proposed financial value are unchanged. These are new read-only proposals; no saved review or case data was replaced.

Flags fall 111 to 49. Overlapping remaining categories: 19 amounts, four balances, 16 dates, ten running-balance differences and three direction conflicts. The five extra running differences are retained. Thirty-nine focused money tests pass. A separate 48-cell synthetic raster experiment accepted nine cells with no wrong accepted values, leaving the other cells unresolved; it is not a broad accuracy claim. Records: `andrews-line-*`, `andrews-money-line-experiment.json`, `money-line-synthetic.json` in ignored `context-audit-final/`. The PDF hash is unchanged. All commands are finished. No push/server restart, and the ten-minute continuation remains active.

Next: finish the remaining difficult-file review work. Merrick's 101 flagged rows include 48 missing-year issues, many caused by conflicting or missing summary/header dates. Inspect those labelled original regions before choosing a further bounded change. Preserve existing crop findings and do not rerun unchanged whole files.
...
The 24-page read-only run took 187.6 seconds and keeps all 16 sections and 383 entries. It retains all 23 earlier refinements (22 money and one period header) and adds 21 money readings. All 21 accepted crops were visually checked. Every previously proposed date, amount, sign, balance and description is unchanged, apart from recalculated diagnostics/column addresses. Flags fall 128 to 116; 79 amount, 19 balance, 28 date, five running-check and three direction problems remain (overlapping categories). Original PDF hash unchanged; no case writes or external OCR. Records: ignored `andrews-compact-image-*`, `andrews-compact-new-records.json` and `andrews-compact-accepted.png`.

28 focused money-region tests pass. A new synthetic database test confirms that a newly separated damaged-date payment is blocked until corrected, retains its original cells and imports exactly once on retry. A date-crop experiment on the remaining 28 date problems found conflicting readings; no automatic transaction-date change has been adopted. Compact date-crop results and their visual sheet are recorded separately for the next bounded step. All diagnostic commands are finished. D remains open. No push or server restart; latest deployment remains `23039a67` and the heartbeat remains active.

## Andrews missing pages and payment boundaries recovered, 17 September 2026

After `4562bd90`, local proposal v19 recovers an unbranded opening only with a share heading, continuation notice and immediately adjacent branded page 2 printing the same account/period. A bounded image pass rereads a damaged full-period date beside its readable partner. The smaller collection's November date was visually checked as 11/01/20. Account applications/agreements and the bordered fee-only page remain accessible information pages. Split damaged dates no longer cause recognised payment rows to join the previous description; their dates remain flagged.

Read-only result: the smaller collection has 16 sections and 383 entries versus 14/296. Eighty rows come from four previously unassigned transaction pages, plus seven recovered lines on already handled pages. Those four pages and all seven crops were inspected. All existing date/amount/sign/balance values, source cells and section IDs are preserved; descriptions and running diagnostics change to reflect the newly separated payments. Flags rise from 89 to 128. All prepared pages now have a recognised purpose, which is not a completeness or accuracy guarantee. The larger collection retains 44 sections and adds 24 genuine payment rows, 2,041 to 2,065; every added crop was visually checked and existing values preserved. Original files and case data unchanged.

Validation: 31 affected backend tests, eight catalog tests, 54 date/money OCR tests and the information-page UI test pass. The final header-date tests also cover either period endpoint and leave readable dates alone. Guides regenerated. Private comparison records and images live in ignored `context-audit-final/andrews-*-grouping-*` and `andrews-grouping-*`. The newly recovered rows still contain substantial genuine field problems; D remains open. No whole suite/build/server restart needed for this checkpoint yet. No push; latest deployed remains `23039a67`.

Rejected experiment: the official Tesseract best English model improved some fields but worsened other sampled dates and changed a row count. It is only under ignored `ocr-model-comparison` and has not changed runtime configuration. Next work should reduce the specific remaining damaged dates/money through measured original-image comparisons, not repeat whole-file OCR or hide remaining errors. All diagnostics are finished. The ten-minute heartbeat remains active; deployment is held until completion and the current URL question remains pending.

## Combined local build and workflow check, 17 September 2026

Production build `2117ae6` (Mighty Waffle) passes with the existing bundle-size warnings. 78 targeted frontend tests across batch/import, selected-row corrections, reassignment, earlier review recovery, shared checks, register and reference components pass. The backend combined check ran 77 tests: 75 passed initially; two stale fixtures were corrected and their targeted reruns pass. The overlap mock now supplies evidence_file_id. The multi-period request carries the explicit undated-charge flag, no longer invents a date, and asserts that each of two charges retains its own period-end ordering date. Product code did not need a change for these failures.

Compiled-app browser checks reused completed synthetic cases without repeating imports. The three-statement batch opened exactly 36 payments, exported the scoped table and cleared its filter. Two reference payments and their original PDF opened; the saved question in Findings survived reload. Both completed transaction years remained in Transactions and the updated guide opened in its modal. No JavaScript errors. Records: `financial-completion-scope-browser.json` and `financial-completion-investigation-browser.json` under ignored `data/local-runtime`. The preview now includes local changes through `2117ae62`; this is not a deployment.

Remaining: difficult-scan review is still too heavy for the benchmark. Current image readings retain 101 flagged Merrick rows and 89 flagged rows in the smaller Andrews collection. Test local OCR quality improvements against the recorded source crops before adopting them; do not loosen source or sign checks to reduce counts. Complete the remaining D acceptance work before final push. The deployed URL question remains pending and the old IP is unrelated. No push; the ten-minute continuation schedule remains active with the intermediate deployment canceled.

## Andrews damaged money rereading verified locally, 17 September 2026

After local commit `43491a77`, the image-reading engine adds a bounded second reading of damaged Andrews amount/balance cells. Only recognised bank/title and measured payment rows qualify. Complete matching readings are required, conflicting digits/signs stay flagged, ordinary withdrawal signs cannot be dropped, and no amount is derived from balances. Source words, geometry and all accepted observations remain in provenance. The runtime fingerprint includes the new helper. At most 40 cells and 30 additional seconds per page, within its existing deadline. Existing readable fields remain unchanged.

The 24-page read-only run retained 14 section identities, 296 transactions, all row addresses and inclusion choices. It recovered 22 cells, all compared visually: ten amounts, twelve balances. Flags fall from 104 to 89; recovered balances expose three extra arithmetic differences. Every existing proposed value is unchanged except recalculated diagnostic differences. A dropped-minus crop result was rejected and regression-tested. No private evidence/import writes, external OCR or changes to the original PDF. Remaining difficult-file flags are still open work; do not claim universal extraction accuracy.

22 focused amount-region tests and 53 affected PDF/geometry/text-origin/manifest checks pass (one expected-response fixture updated for the retained refinement list). User/testing guides and the difficult-file audit record have the exact behaviour and limits. Diagnostics are under ignored `context-audit-final/andrews-refined-image-*` and `andrews-money-preservation.json`. The whole-file reader finished; do not repeat it without a new reason. The larger Andrews collection has not been reread with this helper. Next: combined workflow/build acceptance, then remaining justified difficult-file improvements. No push/deploy while Alex works. Last pushed is `23039a67`.
...
- [x] Individual and background bulk imports share this handling. Server binding prevents a caller from marking an ordinary unreadable date as an undated charge. Drafts retain the marker. Date edits clear it, and affected proposal revisions change rather than silently rewriting an earlier saved review.
- [x] Transactions and transaction details show **Date not printed**, with the statement end separately labelled. A later printed-date correction removes the sorting fallback from the new version, preserves the original reading/source and updates the ordering basis.
- [x] Real Capital One regression is read-only: unchanged 52 periods, 301 entries, amounts, source cells and printed dates; 29 unnecessary interest-date flags removed. One transaction date remains flagged. Closing arithmetic remains 51 matches and one unavailable. This is a rule regression, not independent transcription of all entries.
- [x] Focused checks: 13 initial backend tests, a continuation-page case, eight shared arithmetic cases, the existing date-correction case and the final background-import test pass. Eleven draft/bulk-correction frontend checks and one new import workflow test pass. TypeScript and scoped lint pass. No full suite or production rebuild was repeated.
- [x] Actual local browser: synthetic case `c8911f9c-f864-4ca5-a443-7400f3784b46`, batch `ccf08e05-72e8-4550-b015-495e9d5315b5` imported three card entries with no date decision. The interest retained a blank date, appeared correctly in Transactions and reopened its original PDF. No JavaScript errors. Import is complete: do not rerun the writer. Ignored record `data/local-runtime/undated-interest-browser.json`. Light-mode screenshot 39 and both guides updated.

Two browser assertions initially assumed a text node/dialog structure that the actual inline detail panel does not use. They were corrected against screenshots; imports were not repeated. The initial worker-test fixture wrongly retained its base EUR metadata for a USD card; fixing that artificial fixture allowed the final worker check to pass. Neither was resolved by weakening application validation.

Local API restarted for these changes, port 58002, exec session 88782, log `data/local-runtime/financial-undated-api.log`. Dev UI 55174 includes this work; compiled preview 55175 still represents the previous pushed build. Continue pre-import duplicate warnings/decisions, referenced accounts and Findings, remaining difficult-document checks and wording. **No further deployment before complete task readiness.**

## Authorised release and complete batch checkpoint, 17 September 2026

User instruction: **push now, then in 1–2 hours, then continue to completion**. First push succeeded at approximately 15:25 Dublin: `97e1624d..23039a67` on `origin/integration/evidence-main-reunion`. The second checked release is authorised for **16:25–17:25 Dublin today** (15:25–16:25 UTC). The existing ACTIVE ten-minute heartbeat `continue-loupe-development` now records that window and resumes the remaining A–E plan afterwards. Do not duplicate releases or push failing/unchanged code merely because a timer fires. Record the next hash and time here.

- [x] Production build passed at this release checkpoint, label `cheeky-pancake (23039a6)`. Existing bundle warnings only. One Alembic head: `20260917_financial_batches`; deployment already upgrades migrations and starts the batch worker in the backend lifecycle.
- [x] Fresh synthetic browser journey in compiled preview: selected an Evidence parent folder plus an already-included file, deduplicated to three PDFs, entered Financial automatically, imported two ready periods (24 payments), jumped to the remaining date error beside its PDF, saved a correction, cleared browser storage, recovered the server draft and imported the final 12 payments. Refresh retained 36 payments and three completed statements with no JavaScript errors.
- [x] **Open imported transactions** showed exactly the three batch sources and 36 rows, and produced a scoped ZIP download. This uses deliberately prepared synthetic Evidence files and does not measure fresh raw-PDF extraction accuracy.

Synthetic case `ba3c25ae-8361-496b-883c-62d35dce94bc`, batch `e276c311-c54c-4688-875d-2b2b2917b29a`. All three imports are complete; do not repeat the writer. Ignored records: `financial-checkpoint-browser.json`, `financial-checkpoint-scope-browser.json`, `financial-checkpoint-scope.zip` under `data/local-runtime`.

Deployment is **not verified**. The previously recorded IP `104.196.62.215` now serves an unrelated transport website; port 5174 refuses connections. An asynchronous question asks the user for the current Loupe URL. Do not send credentials or case data to that old address. This does not block local development or establish that the Git-triggered deploy failed.

In progress: legitimate undated Capital One interest charges should retain no printed transaction date and use the existing explicit statement-end ordering context. Generic missing dates and unreadable amounts must still block; source fields, saved drafts, imports, later corrections and investigation displays must agree. No implementation changes for this task had been made when this entry was written.

## Main statement register date gaps and duplicate review, 17 September 2026

The user asked for the current position, a to-do list and an ETA. Explained that the core DocuClipper-style folder/batch/review/import journey works locally, while difficult-file exceptions, pre-import duplicate warnings, referenced accounts without statements, Findings connections, wording and final workflow/deployment checks remain. Estimated 6–9 hours of active work to a locally verified, ready-to-push version and 30–60 minutes for deployment checks after a push; this is an estimate, not measured completion or a guaranteed deadline. The ten-minute continuation schedule remains ACTIVE. No push requested or performed; last pushed remains `97e1624d`.

- [x] Main **Statements & accounts** register now loads imported statement date checks automatically, showing accounts with gaps, overlapping periods and dates still needing attention. Account labels include holder, reference, bank and currency. Counts explicitly cover the current page when more than 25 accounts are present. Pending files are not described as imported coverage, and unavailable results are never shown as a successful no-gap check.
- [x] **Review dates & statements** opens an account-specific window with coverage first, requested-date checks, retained original filenames/PDFs, balance checks and navigation to that account's payments for the checked dates. File metadata is case-scoped on the server.
- [x] **Check duplicate imports** opens the existing comparison immediately from the register. A deliberate exclusion or restoration retains the original files/history and refreshes date coverage and payment views. This is post-import duplicate review. The pre-import duplicate task in D remains open.
- [x] User guide and team testing steps describe these exact paths. New light-mode screenshot 38 contains only synthetic data. Portable guide, public guides and test pack rebuilt. Source/public screenshot bytes and absence of em dashes verified. Duplicate decision wording now describes payments and Transactions rather than internal ledger status.

Validation: four focused backend coverage checks and twenty focused frontend register/coverage/comparison checks passed. One affected FinancialPage navigation test passed. TypeScript, scoped ESLint and a production build passed. Seven duplicate-decision unit tests passed after the wording change. The duplicate-decision browser regression initially lacked the case-editor context and therefore correctly showed no write actions; its fixture now supplies that context. That focused browser test now passes through both exclusion and restoration. No full suite was run. The sandbox initially blocked the browser test's local port; the approved run could launch it.

Actual browser evidence: new synthetic case `39fbc605-8b3c-4e80-930f-7bf0ecc724bd`, account `8ccb18fa-6bc1-4566-96f0-7bd6e45e0254`, has January and March statements plus a repeated March copy, each with one artificial payment. The register shows February 1–28 missing and one overlap. A December 1–April 30 check gives 151 requested days and 89 uncovered days in three gaps. Source navigation and date/account-scoped payments opened correctly. Excluding the repeated March copy reduced the coverage count to two periods and removed the overlap; restoring it returned three periods and the overlap, including after refresh. The compiled app also opened the original PDF viewer. No application JavaScript errors. Record `data/local-runtime/financial-register-dates-browser.json`. Both exclusion and restoration are complete; do not rerun that writer. The case ends with the sample copy restored.

An earlier synthetic setup used the all-letter reference SYNTHETIC-REGISTER. Existing account identity rules did not join those three documents, so it was unsuitable for the same-account coverage scenario. That original fixture was preserved unchanged. The successful fixture uses a full numeric artificial reference and the existing normal import path, with no identity-rule or database repair. Original real evidence was never changed.

Local API: port 58002, exec session 28415/PID 89715, log `data/local-runtime/financial-register-api.log`. UI 55174, compiled preview 55175, engine 58003. The compiled build contains the register functionality but predates the final duplicate wording and guide additions. No further production build is needed until the next meaningful integration checkpoint. Preserve unrelated untracked files, archives and real statements.

Next concrete priority: reduce avoidable difficult-file flags, particularly undated statement interest/fees. Inspect the existing `ordering_date_context === "statement_end_ordering_only"` support before introducing any date behaviour; retain the absence of a printed transaction date rather than inventing one. Then address pre-import duplicate warnings, account references without supplied statements and direct Findings actions. Keep all completed checklist entries checked and do not claim deployed verification before a later authorised push.

## Difficult Capital One dates and split descriptions, 17 September 2026

The user asked where the DocuClipper benchmark stands. The response distinguished the locally working workflow from the live version, confirmed the ACTIVE ten-minute schedule, and named the remaining difficult-file and investigation connections. Continued the read-only audit started after `a89c3f7b`. No push requested or performed; last pushed remains `97e1624d`.

- [x] Added measured mapping for split Capital One descriptions. Six previously missed amounts now map to the actual right-aligned Amount column; description fragments are joined with each original citation retained. Advertisements beyond the table remain in the source cells but do not enter payment descriptions. Missing, inconsistent or ambiguous geometry stays unresolved; unreadable amounts are not repaired by guessing.
- [x] Fifteen transaction dates just before the billing period now retain their printed dates using their separately printed in-period posting date to identify the year. A conservative 31-day lookback handles New Year. More distant dates, invalid dates and purchases later than their posting date remain flagged. Fees use the same rule. Both dates survive import separately.
- [x] Whole-document read-only regression retains the exact 52 periods, 301 entries, row IDs, inclusion choices and source cells. Closing arithmetic improves from 45 matching / 7 unavailable to 51 matching / 1 unavailable. Flagged periods fall from 36 to 29. Twenty rows changed, including one with both an amount and date fix. Remaining row flags: 29 undated interest charges and one transaction before its cycle with no posting date. These are not silently assigned invented dates.
- [x] Independently read physical PDF pages 57, 87, 191 and 217 visually and transcribed expected counts/totals separately. All 27 entries in that sample agree. Four included interest charges still have no printed date. This is targeted sample evidence, not full-document ground truth or independent testing of all 301 entries.
- [x] Added a tracked aggregate audit report with no personal statement data; user guide explains separate dates and source-cell preservation. Private original renders and reproduction scripts remain ignored under `data/local-runtime/capital-inspection/audit/`.

Validation: 24 distinct focused backend tests pass (seven new geometry/date tests, affected layout/card tests and one actual database import/retry regression). Three layout-context frontend tests, TypeScript and scoped ESLint pass. No full suite or production rebuild was repeated for this small extraction/context change. The first test invocation found pytest unavailable; the repository's unittest runner completed the checks. One synthetic fixture initially had invalid test-only source locators and was corrected before the integration check passed.

Remaining priorities are unchanged: finish avoidable flags and summary/disclosure separation across supplied files; make a recorded date decision for undated statement charges practical; expose duplicate/overlap and missing-period decisions in the main register; link referenced accounts without statements to their payments and Findings; finish wording and later authorised deployment verification. Keep completed checklist items. Do not reimport existing synthetic fixtures or publish private audit data. The compiled preview still represents `a89c3f7b`. The local API was restarted with the extraction fixes on port 58002 in exec session 48500, log `data/local-runtime/financial-card-audit-api.log`; use development UI 55174 for the updated context wording. The sandbox initially prevented port access; the approved local restart succeeded.

## Selected-row account and period reassignment, 17 September 2026

This checkpoint follows local commit `e1beb929`. The ten-minute continuation schedule is active, and no push was requested or performed. The existing accepted investigation workspace remains intact.

- [x] **Correct several rows** now includes **Move to another account or period** for extracted transactions in recognised periods of the same PDF, before either statement is imported. The user selects rows and a destination, explains the move, and previews both accounts/periods, transaction counts and balance checks before saving.
- [x] A file-level assignment retains each original row ID, reading, source cells, PDF location and reason history. It removes the row from its prior review exactly once and allows a later move back. Current corrections are saved together with the move. Individual and bulk reviews retain their respective edits, including different saved descriptions. Both reviews and affected batch counts/checks update atomically. Older reviews are retained in history.
- [x] The preview binds the selected rows, current unsaved values, source readings and all affected saved reviews. Concurrent saves, a queued import, stale readings and changed selections refuse the move instead of overwriting changes. Case-edit permission is required. Balances, totals, manually entered rows, already imported statements and currency conversion are not part of this action.
- [x] Imports after reassignment preserve source provenance and do not mistake two deliberately assigned periods sharing a table for duplicates. A focused test imports the receiving period first and then the source, yielding 4 + 2 transactions, with one copy of the moved row. It also retries both confirmations without duplicates.
- [x] User guide and team testing checklist include the exact steps, recovery expectations and limits. Light-mode screenshot 37 uses synthetic data only. Generated guide and test pack updated.

Validation: six reassignment backend tests pass, covering retained edits, source locations, stale previews, case isolation, controls, separate batch corrections, queued/imported guards, move-back history, changed extraction and both-period import. A printed-balance regression starts with two matching statements and verifies opposite 180.00 differences after moving one payment. The case-edit route check and affected batch-save/progress checks also pass (nine distinct focused backend tests overall). Five focused frontend tests pass, including existing selected-row behaviour and stale move previews. TypeScript, scoped ESLint and production build pass. No full suite was run.

Actual development-browser check: new synthetic case `65bc35d5-06db-4972-a83b-54a833c4c855`, file `c0a0c751-6250-4448-a442-e35eb4d667a9`, batch `3121763d-c0e9-4084-b377-46ac27bf8f45`. One payment moved from one account/period to the other; saved counts are 2 and 4. Reopening after refresh retained the move and its original PDF location. No payments were imported and there were no JavaScript errors. Record `data/local-runtime/financial-assignment-browser.json`; screenshots are synthetic. A reverse-move preview for the guide was read-only, not saved. Do not rerun the saved move. The compiled-app check is recorded in that same ignored JSON record.

Local API was restarted with final changes in exec session 83564 on port 58002, log `data/local-runtime/financial-assignment-api.log`. UI 55174; compiled preview 55175 now contains this checkpoint. Preserve unrelated untracked files and original PDFs.

Remaining work: read-only difficult-file accuracy and false-flag audit; summary/disclosure separation; visible duplicate and missing/overlapping-period decisions in the main register; account references without supplied statements linked to payments and Findings; wording consistency and later authorised deployment verification. This reassignment action is pre-import and uses existing detected account/period sections. It does not create new undetected periods or move already imported ledger records. If a reread changes rows after reassignment, it must use a new file version with saved-review comparison rather than guessing a new row location.
...
- [x] **Correct several rows** selects transactions across pages, previews before/after values, and applies exclusion/restoration, credit/debit switches, dates or paid-by/paid-to names with one reason. Previous row explanations remain. Source controls are excluded from this bulk action. An outdated preview is refused. Original printed cells remain unchanged and shared arithmetic recalculates.
- [x] Individual **Save progress** now persists even incomplete corrections to the case. Evidence-row locking plus revision checks prevent another reviewer being silently overwritten. The draft is separate from the original reading and imported ledger. Existing imported transactions use their established correction path. Starting a new batch from an individually saved draft retains its corrections and unfinished fields.
- [x] The explicit parent period of a reprocessed file can recover an individual saved draft when its extraction revision matches. Otherwise the previous values are shown separately and must be compared. An in-place reading change retains the earlier saved request in history. Unmatched period IDs, batch-only drafts and broader reprocessing recovery still need work, so the full D item remains unchecked.
- [x] The user guide and test guide now cover individual server saves and selected-row previews. Images 34 and 35 use artificial data in light mode.

Verification: the prior final adjustments passed 35 targeted frontend tests plus production build. The progress change passed 93 backend test cases from four focused files (pytest also collected imported fixture classes, so avoid repeating that expanded collection); the five final new progress/batch-seeding tests were then run by explicit class/test IDs and passed. Three selected-row tests and three individual-progress/conflict tests passed. Scoped ESLint, TypeScript and the final production build passed. No full financial suite was run.

Actual browser evidence: isolated synthetic case `12af9eda-9d40-4756-9483-f4cd484049d7`, file `5b517eac-47e9-42ef-a3b8-3506d66db1f5`. An incomplete date was saved, all session storage cleared, and the file reopened from the register. The saved date was restored from the server. Two selected transactions were previewed and switched, producing running-balance differences; switching them back restored matching checks. The final correct values and explanations were saved to the case. Nothing was imported. The compiled app reopened that saved review, displayed a two-row correction preview and loaded the revised guide. No browser JavaScript errors. Records: ignored `data/local-runtime/financial-progress-browser.json`; screenshot 35 came from `financial-selected-corrections.png`. Earlier three-statement/36-payment browser imports remain intact and must not be repeated.

Local API is now exec session 42055, port 58002, with output redirected to ignored `financial-progress-api.log`; development UI 55174, compiled preview 55175, engine 58003. Do not dump whole API logs. New single-review draft storage uses Evidence metadata with revision locking and needs no new migration; durable batches still need the existing `20260917_financial_batches` migration.

Next concrete steps for continuation:

1. Complete D recovery for saved batch reviews and reprocessing whose statement IDs change. Surface unmatched earlier reviews explicitly; do not auto-map different source rows or claim they were preserved in the new reading.
2. Implement selected-row account/period reassignment with server validation, source retention, audit history and recalculated checks in both affected periods. Whole-statement metadata correction already exists; this is the missing row-level operation.
3. Audit remaining Capital One/other supplied-file flags read-only, distinguish real uncertain transactions from summaries/disclosures, and record each false flag or omission. Do not claim the 52-period/301-row extraction is independent ground truth.
4. Bring overlap/duplicate decisions and missing-period checks into the main register. Existing services exist; integrate them rather than rebuild the accepted investigator workspace.
5. Complete E: batch **Open imported transactions** currently opens the general table. Add exact imported-source scope, clear labels and reset controls with correct exports. Single import already scopes by account/source. Add missing-account references with underlying payments and save those questions to Findings.
6. Finish consistent wording and affected-path validation. Deployment verification waits for a future authorised push; never push just because the heartbeat fires.

## Scheduled processing benchmark implementation, 17 September 2026

User instruction: continue all next development steps and use a schedule. The existing heartbeat `continue-loupe-development` is now ACTIVE every 10 minutes without the old expired cutoff, renamed **Complete Loupe financial processing**. It resumes the unfinished A-E checklist in `docs/docuclipper-benchmark-2026-09-17.md`, avoids duplicate active work, uses focused tests, keeps completed items checked, and pauses only when the authorised benchmark and verification are complete. No new question or push permission is needed to continue local implementation. It must not automatically push merely because the timer fires.

This local checkpoint completes shared exact arithmetic across batch and individual imports: opening/closing controls, date-ordered running-balance intervals (including reverse order), explicit supported printed credit/debit totals, source links, unavailable results, debounced rechecks and rejection of stale responses. Single and bulk import enforce the same differences. An investigator can explicitly record why a discrepancy remains in the PDF, bound to the exact reviewed values; later value changes invalidate that acknowledgement. The duplicated browser arithmetic helper was removed. Post-import running checks no longer truncate at 1,001 rows or refuse periods above 1,000; a 25,000-row regression checks the final correction too.

The workspace now separates account and period selectors, offers period search and counts, edits directly beneath the selected printed row, marks corrected/checked rows, and retains original cells and PDF highlights. Problem navigation is separate from transaction/page navigation. Batch **Save progress** accepts incomplete drafts without marking them ready; optimistic revisions protect another reviewer's changes. **Save and open next/previous problem** crosses period/file boundaries and list pagination. Returning to the batch preserves its page. Guide and testing checklist describe these actions; image 34 uses only synthetic data in light mode.

Validation: 47 focused backend checks passed for arithmetic, incomplete drafts, folder batches, source controls and large running comparisons; 11 focused route/recovery checks also passed. Forty-one focused frontend checks passed for printed tables, review, period/account selection and batch interaction; the two async arithmetic-hook checks passed separately. TypeScript, scoped ESLint and production build passed before the final small validation/copy adjustments; final validation is recorded below when complete. Existing React act warnings occur in a few older fixtures. No broad full suite was run.

Actual browser verification: new synthetic case `c008b0ad-461a-4a36-9278-5b99db893ad9`, batch `46887499-a8be-47c7-a8a1-65ddff70a612`. It saved an arithmetic mismatch, cleared browser storage and restored the server correction, saved an incomplete date without allowing import, restored values, navigated to another file's flagged row, edited beside the printed row, and imported all three ready statements. Final persisted result: 36 transactions, 3 imported periods, 0 pending, 0 attention, no browser JavaScript errors. Do not rerun these imports. Ignored evidence: `data/local-runtime/financial-checks-browser.json`, `financial-inline-review.png`, `financial-checks-ready.png`. The API was restarted in the isolated local environment on port 58002, exec session 33004. UI development 55174, compiled preview 55175.

Real-data regression was read-only against `data/local-runtime/capital-inspection/native-sources.json`: 52 periods, 301 proposed payments, 45 matching closing checks, 0 differences, 7 unavailable; 0.187 seconds for catalog plus all-period checks. Original PDFs unchanged; no real imports. This does not resolve independent extraction accuracy or user acceptance.

Remaining: finish checklist A consistency and deployed verification; B recalculation after account/period reassignment; C remaining save-state/recovery paths; D difficult-document false flags, selected-row correction previews, explicit reassignment and reprocessing conflict handling, duplicate coverage decisions; E scoped investigation links, main-register coverage, missing-account references and saving those questions in Findings. Continue from this list, not from historical requests to stop for an early preview. Preserve unrelated untracked documents and archives. No deployment has been performed by this checkpoint.

## DocuClipper research and processing benchmark, 17 September 2026

The user requested deep research into DocuClipper and directed that its ease of financial document processing should be a benchmark for Loupe. Research and proposed acceptance criteria are recorded in [docuclipper-benchmark-2026-09-17.md](docuclipper-benchmark-2026-09-17.md). This is a research/documentation checkpoint, not an implemented redesign or push. The existing uncommitted development below remains intact.

Reviewed primary documentation and the public read-only Financial Investigator demo. No private statements were uploaded externally. The benchmark preserves the accepted investigator workspace and focuses on automatic per-account/per-period checks, one bulk confirmation for ready statements, a compact PDF/table review with nearby corrections, and direct next-problem navigation. Important local gaps identified: pre-import versus viewer balance checks differ; existing running-balance diagnostics are separate and limited to 1,000 period rows; separate printed credit/debit subtotal checks are not integrated into batch readiness; correction controls and long period labels still add user effort. Proposed work and concrete completion scenarios remain unchecked in the research report. Vendor accuracy claims are not independently verified and public multi-period/retention descriptions have caveats documented there.

## Evidence folders to persistent Financial bulk import, 17 September 2026

Current branch remains `integration/evidence-main-reunion`; last pushed commit remains `97e1624d`. This checkpoint and the preceding intake/review work are local, uncommitted and unpushed. No push was requested for this batch. Preserve unrelated untracked business documents, archives, backup files and all real statements. The old automation remains paused.

Completed work, retained as a checklist:

- [x] Evidence **Send to Financial** and Financial **Choose from Evidence** browse folders and select nested PDFs. The reviewed list is deduplicated and non-PDFs are skipped. Starting uses that exact reviewed file list, closes the picker, and navigates directly to Financial's batch page.
- [x] Durable server processing reuses existing readings or starts the existing financial preparation job without overwriting completed general Evidence analysis. Files and each recognised statement period have saved status. Leaving or reloading the browser does not stop the batch. Worker leases prevent duplicate processing and renew during longer jobs. A PostgreSQL foreign-key lock found during real browser testing was fixed by releasing the evidence-row lock before writing batch items through another session.
- [x] Readiness uses the existing import validator, source revisions, row review reasons and available balance checks. Unresolved dates/amounts/currency, reading failures and balance differences stay out of bulk imports. Missing independent balance controls are plainly labelled unavailable; they alone do not block otherwise valid records. Receipts retain their separate review.
- [x] One confirmation queues only the ready statement list displayed to that user. A changed ready list must be refreshed before confirmation. Later-ready statements are not silently added. The background import uses the existing audited/idempotent statement writer, preserving successful periods if another fails and preventing duplicate payments after a restart/retry.
- [x] Whole-batch attention filter, exact row links, original PDF comparison, saved review corrections, **Save for bulk import**, and return to the batch. Corrections persist server-side. Saved-review revision checks prevent silent concurrent overwrites. Browser drafts are separated by the server review revision. Batch reviews remain tied to their selected file/period.
- [x] File processing errors stay visible with **Open file review** and **Retry this file**. Unknown currencies can be supplied from the batch. Imported transaction lists refresh when a batch finishes; **Open imported transactions** leads into the investigation table.
- [x] User guide and testing checklist explain this full workflow. Published a light-mode screenshot containing synthetic data only (`33-financial-bulk-import.png`). Regenerated portable and in-app documentation.

Validation: seven focused batch persistence/readiness/confirmation tests, eight file-intake tests, eight route authorization tests and three period balance tests pass (26 across focused runs). Thirty targeted frontend tests pass across the review/picker/batch groups, including the corrected focused panel rerun. Scoped lint and TypeScript/production build pass. A disposable PostgreSQL database verified both a fresh migration chain and repeated upgrade to `20260917_financial_batches`; existing databases were not used for that migration test. The new migration was also applied to the isolated local app database. `deploy/deploy.sh` already runs `alembic upgrade head` before deployment completes.

Actual browser verification used two separate synthetic local cases, never real financial imports. In each, selecting a parent folder selected three statements including its nested folder, navigated automatically into Financial, imported two ready statements (24 transactions), retained one flagged statement, opened its exact date correction, saved it, restored the correction after clearing browser-only drafts, then imported the remaining 12 transactions. Final result: 3 imported statements, 36 payments, no browser JavaScript errors. Records are in ignored `data/local-runtime/financial-bulk-browser.json` and `financial-bulk-browser-final.json`. The final synthetic case is `2b76dc7d-f280-4cff-8bf9-0c138e9c9c17`; its batch is `5e2a571f-f434-4c45-85aa-cde3428b718a`. Synthetic originals use separate paths so the earlier test evidence was not overwritten. The compiled browser check is recorded in that final JSON when complete.

Local services: development UI 55174, compiled preview 55175, API 58002 (exec session 62618), engine 58003. New files include batch models/migration/service/router endpoints, `FinancialBatchPanel`, its tests and review context. The earlier in-memory Evidence sending queue was replaced with this durable flow, leaving a small `lib/evidence-selection.ts` response schema. No broad financial test suite or real-data imports were run for this change. Deployed verification awaits a future push; this is not a claim that the server has these changes.

## Statement intake, import checks and period navigation, 17 September 2026

Current branch is `integration/evidence-main-reunion`. Last pushed commit is `97e1624d` (light-mode guide screenshots). The changes in this checkpoint are local, uncommitted and unpushed. The daytime automation remains paused. Preserve unrelated untracked business documents, archives, PDFs and backup files.

Completed work, retained as a checklist:

- [x] Remove an unrelated PDF from Financial without deleting Evidence. Removal is case-wide and revision checked, with a Removed files list and restore. Files already linked to recorded periods or transactions cannot be removed this way. No migration.
- [x] Browse Evidence folders from Financial or Send to Financial in Evidence. Select PDFs or folders including descendants, inspect the deduplicated list, and queue preparation. Existing readable files are reused; a processed file needing financial text gets a separate reading without erasing its general Evidence processing. Individual failures remain visible.
- [x] Explain disabled confirmation with the exact missing fields and flagged rows, direct correction links, and an explicit checked-against-PDF action for unchanged valid flagged values. Clean statements still require just one import confirmation, not line-by-line acceptance.
- [x] Recognise the World Elite Mastercard heading shown in the user's second Capital One example. An unseparated multi-statement Capital One reading no longer presents thousands of summary/information lines as an import; it explains the problem and offers a fresh PDF-text reading.
- [x] Direct statement-period dropdown and Previous/Next period controls. Each recognised period receives read-only checks of its extracted transactions against independently printed opening/closing balances. A match does not clear flagged dates or amounts, and missing controls are labelled unavailable. The original-extraction labels do not claim to describe later edits.
- [x] Previous/Next transaction controls, counter and left/right keyboard navigation. Navigation opens the correct source page, highlights the measured row and scrolls the extracted table. Headings and excluded rows are skipped. The first view starts on a transaction page where available; remembered page choices remain available.
- [x] Upfront current-review checks, automatic recalculation after edits, and a link to confirmation or required corrections. Normal credit-card/share information is under About this statement rather than presented as an error.
- [x] Guide and step-by-step testing checklist updated and their public/portable versions rebuilt. No real financial screenshots were published.

Validation: focused file-intake/router/import refusal checks passed (16); World Elite/catalogue/layout checks passed (15). New all-period balance checks passed (3), and the existing two-period import isolation check passed. Navigation/review/picker tests passed (28), printed-table tests passed (5), and measured-row-location test passed (1). Earlier new file action, evidence picker and queue checks passed. Scoped lint, TypeScript and one production build passed. No full suite was run. The build retains the existing bundle-size warning.

The exact supplied `/Users/neilbyrne/Downloads/001052-001273 Statements (1).pdf` is byte-identical to the existing test PDF (SHA-256 `22e405327ccfa1e33acbc7710db92ab4a4ea7d16027fd659cb55632855cc2c26`). Fresh native extraction and visual inspection covered its 222 pages. It contains 52 periods for endings 8160 and 2602, not the 51 periods/end-9392/201-page document in the DocuClipper screenshots. Fresh extracted checks find 301 transactions: 45 periods have matching opening/closing controls, seven lack sufficient readable controls. Thirty-six periods contain one or more individual flags, including uncertain dates, unreadable amounts and undated interest charges. These are not independently verified ground-truth accuracy measurements. The first period has three payments, starting on physical PDF page 3, with matching balances and no flagged rows.

Local browser verification reused synthetic records for folder selection/deduplication, file removal/refresh/restore and blocker navigation. Original synthetic file placement was restored; no financial imports were performed. The supplied real PDF was used read-only to verify all 52 choices, three first-period transactions, direct period switching and transaction navigation. Browser records are `data/local-runtime/financial-file-browser.json` and `financial-period-browser.json`; private source screenshots and extraction checks are in `data/local-runtime/capital-inspection`. Never publish those real-source images. Synthetic unit tests cover single-confirmation import and restoration of corrections between periods.

Local API was restarted to load this batch (exec session 59497); UI development is 55174, compiled preview 55175, API 58002, engine 58003. The latest built UI checks and source-image verification are recorded in the browser JSON. No server deployment or team acceptance is claimed. The separate 201-page World Elite document was not supplied locally; its header and multi-period behaviour have a synthetic regression, not full-document validation.

## Light-mode guide screenshots, 17 September 2026

The user requested light-mode screenshots for the docs and guides. All 29 distinct screenshots referenced by the user guide (30 appearances) and all three testing-guide screenshots now show the current application in light mode, using existing synthetic cases. This includes the investigator workspace, original statement review, reports and the older specialist/manual tools. Captions now match the captured examples, and the Trends instructions name the current teal and rose colours. No real statement or case data was published.

Rebuilt the portable user-guide HTML, both in-app Markdown guides and their public images, the standalone testing guide and its downloadable ZIP. Source/public/embedded image bytes match; the ZIP includes the updated instructions and three images with the unchanged checking and savings PDF fixtures. Both actual guide modals loaded every image and contents link, and the portable guide and extracted testing pack loaded all their images. No case writes or application JavaScript errors occurred during this verification. No full test suite was run for this documentation change. Capture details and results remain ignored in `data/local-runtime/guide-light-capture.json` and `data/local-runtime/light-guide-verification.json`.

The preceding colour pass is pushed as `413ed98bf7e6fd62aaf306c8b267596a94fe1fac`. The user has now requested a push of this checked documentation refresh. Publish it on the existing `integration/evidence-main-reunion` branch through the normal auto-deploy process; deployed asset verification remains separate from the completed local checks. The daytime automation remains paused. Preserve unrelated untracked material and existing synthetic records.

## Financial colour pass, 17 September 2026

The user accepted the investigator layout and asked for more colour, explicitly allowing colours beyond Loupe when they remain consistent with its style. Implemented a finance-only palette: teal incoming payments/card credits, rose outgoing payments/card charges, blue accounts and source information, amber questions/review work, and muted purple saved work/name profiles. Loupe carmine remains on actions and navigation. Totals, payment tables, connections, profiles, Trends, Findings, file status and source/comparison views share the colours. Labels and original PDFs remain intact. A browser check also found and fixed narrow navigation overlapping the page heading.

Reused the existing synthetic Nexus and checking/savings cases without uploading, importing or writing findings. Light/dark screenshots and contrast readings are in `data/local-runtime/financial-colour-verification.json`; coloured text/fill pairs exceed 4.5:1 in both themes. `financial-colour-detail-verification.json` records the 390-pixel navigation check, original PDF visibility and actual colour in the comparison dialog. No application JavaScript errors were observed. Scoped lint and the production build passed; no full test suite was run for this visual change. Existing extraction and larger-analysis limitations are unchanged.

The preceding investigator implementation was committed and pushed as `27aa3f1f4de05b53133c6bfa2d6cf1be876c6511`; origin matched it. The first server check still showed the previous build, so deployed verification was not claimed. The user has now requested publication of the colour pass; deployment verification remains separate. The daytime automation remains paused. Preserve unrelated untracked material.

## Investigator workspace built and locally verified, 17 September 2026

The user's approved design is implemented in the real financial application. Main pages are Overview, Statements & accounts, Transactions, People & businesses, Follow money, Trends and Findings. The main file register opens the retained PDF/extraction review. Compact payments have contextual originals, selection comparison, nearby-payment search, follow-up markers and selected CSV downloads. Profiles and real chronological charts lead into the same comparison/finding workflow. Findings have editable explanation/type/progress/next action/recorded owner and preserve their original cited versions. Existing reports, corrections, identity links, specialised analysis and import history remain accessible. No push or deployment of this redesign has occurred. The daytime automation remains paused.

A compact case-scoped read endpoint indexes financial finding/payment links without loading report bodies or snapshots. It uses existing case-view authorisation and requires no migration. The local API was restarted to include it; its current process at this checkpoint is 54370 (exec session 80149). UI development remains 55174, compiled preview 55175, API 58002 and engine 58003. No provider credentials or private-source files were changed.

Local browser evidence: the existing checking/savings fixture (guide-walkthrough.json) still has eight payments, USD 1,855 in and USD 895 out. It now has one additional synthetic investigation question, revised next action/progress, and one saved report created through the new UI. These are recorded in investigator-report-verification.json. Do not repeat those writers. The clean Nexus fixture 81555656-4bb9-4d9e-b2a1-6bf2b8c571d3 verifies six adjacent receipt/outgoing comparisons, twelve linked payments, six Cayman National Bank payments, twelve calendar months and two December payments. The 2,700-payment case opens the full selected comparison and subsequent pages. The 5,000-payment, 167-page synthetic statement retains page 2 through register return, tab navigation and refresh. Nearby-payment lookup, actual-currency CSV amounts, finding filters and the narrow source overlay passed. No new uploads or imports, no application JavaScript errors in these browser journeys.

Focused checks cover the new comparison/calendar/amount helpers, finding text round-trip, navigation restoration, nearby query completeness and the affected page/source/file/table workflows. The backend index scope/pagination/deletion check passed. Scoped lint, TypeScript and the production build were checked; no full suite was repeated. Final production build passed in 7.72 seconds with the existing bundle-size warning. The compiled preview then passed Overview, all twelve Nexus chart months and the revised guide with no JavaScript errors (investigator-compiled-verification.json). The final affected-component batch also passed all eleven table/source-selection/correction checks, and all forty-three page/nearby-lookup checks passed. Guide/public text, image paths and packaged PDF/image bytes were verified. The guide and testing pack now describe the implemented screens, with updated synthetic screenshots and unchanged synthetic PDF fixtures. Final validation results are retained in local runtime logs and JSON records.

User acceptance of the implemented UI, separate-colleague acceptance and deployed verification remain unclaimed. Existing extraction accuracy questions and the three separate capacity gaps (Transfers above 500 inputs / 1,000 pairs, case-event comparison above 1,000 payments and larger Patterns) are not solved or closed by this redesign. The written approved specification and implementation checklist are docs/financial-investigator-design.md. Preserve unrelated untracked files and do not resume the paused daytime automation.

## Investigator design request after UI rejection, 16 September 2026, late evening

The user rejected the deployed UI as unsuitable for an investigator, specifically Trends, People and businesses, Findings, the missing main-page statement register and the crowded Transactions screen. They explicitly stopped an immediate implementation response, then requested: "please design this from an investigators perspective". The active deliverable is design, not another unreviewed application rewrite or deployment.

The proposed behaviour and actual-product acceptance tasks are recorded in [financial-investigator-design.md](financial-investigator-design.md). A clickable design in the task's visualization directory uses the twelve synthetic Nexus entries from the conversation. It connects Overview, the main statement register, payment selection/comparison/source references, person profiles, chronological monthly charts, evidence-linked findings and a report preview. Its statement excerpt is explicitly a recreation; its saves are local to the preview. No application code or live records have been changed for this design request. User acceptance of the proposed design remains open. Preserve the existing extraction, correction, source, saved-work and reporting functionality when implementing it; the previous functional checks did not establish investigator usability.

The user asked whether to ingest additional files. Existing screenshots and supplied sources are sufficient for the design; no additional upload was requested. Multi-account, currency and relationship evidence will be useful for subsequent validation. The prior documentation update 6a7b9685 is deployed; all eleven public guide/test assets matched local bytes. The daytime automation remains paused. The three previously recorded larger-analysis capacity gaps remain open.

## Financial documentation and testing guide, 16 September 2026, evening

The user's new evening request is to update the documentation inside Financial and provide step-by-step testing instructions. This work does not restart the paused daytime schedule or mark the three larger-case analysis gaps complete.

The existing Financial guide modal now offers User guide and Testing guide, each with contents navigation, retained loaded content and recoverable download errors. The testing guide also opens separately. Its nineteen sections cover a first investigation, further specialist-tool exercises, real/multi-page statement checks, known limits and a results sheet. The downloadable pack contains two unchanged synthetic PDFs, HTML/Markdown instructions and three current synthetic screenshots. The full user guide points to it, updates two core screenshots and corrects obsolete 20-note/20-file report limits. The separate twenty-file upload and claim-support limits remain where applicable.

Actual new local verification: a fresh synthetic case imported six checking and two savings payments after excluding the explicitly flagged synthetic footer. Combined totals are USD 1,855 in, USD 895 out and USD 960 difference. The guide documents that required footer decision rather than implying an unqualified automatic import. The guarded fixture record is data/local-runtime/guide-walkthrough.json; do not repeat its successful uploads or imports. The separate screenshot-only prepared statement is recorded in guide-image-review.json and was not imported. No private real PDFs were published.

The focused guide browser check passed across fourteen tabs, both guides, desktop/phone layouts, contents links, all three test images, failed-load retry, Escape/focus return and preservation of an unsaved case form, with zero financial writes. The compiled standalone guide and extracted offline pack passed image, anchor and narrow-layout checks without JavaScript errors. Source/public/ZIP assets match, the PDF bytes are unchanged, and all referenced files exist. Scoped ESLint, TypeScript/production build and diff checks passed; no full suite repeated. This validates the documentation delivery and sample import, not execution of every testing exercise or authenticated production acceptance.

The remote was fetched without incoming changes. Publish this requested documentation/UI update normally, including the prior local evening-handoff checkpoint. Verify the deployed commit and guide assets after the poll. Keep continue-loupe-development paused; transfers above 500 payments, case timelines above 1,000 payments and larger pattern analysis remain open.
...
The final read-only runtime check found all services, schema and worker healthy. The 2,700-payment synthetic case passed complete totals/export, statement checks, coverage, graph and account-party reads. Three analysis reads refused the unfiltered case under existing capacity limits: Transfers allows 500 payments and 1,000 pairs/reference comparisons; Payments and case events allows 1,000 payments; Patterns shares that input limit and also bounds matches, repeated-name groups and path searches. These separate limits were omitted from the earlier capacity summary and are now explicitly recorded as remaining development in the completion plan. Do not report all large-case analyses as complete.
...
The existing continuation heartbeat is ACTIVE every ten minutes through 18:00 Dublin. The user is away, authorizes necessary development and verified daytime pushes, and does not want permission pauses between segments. Start final integration by 17:00 and pause at 18:00 after recording the evening status. The plan is recorded with completed items retained in [the completion plan](loupe-financial-completion-plan.md).

The earlier capacity commit 02a5a163 and all preceding work were pushed. The remote branch matched the local branch at that checkpoint, and the public server version returned 02a5a16. That is frontend identifier verification, not authenticated deployed-case acceptance.

The verified batch committed and pushed as 2898f70e removes the separate 100-payment linking restriction, supports complete larger linking/history with atomic revision checks and bounded SQL batches, pages large original-name lists, recovers specific whitespace-damaged Andrews readings in v16, and provides the short team checklist plus two synthetic PDFs. See [the daytime release note](releases/2026-09-16-financial-daytime.md) for exact checks and bounds. Focused backend/frontend checks, scoped lint, TypeScript/build and final compiled browser checks passed. No broad suite repeated.

Actual local evidence: 2,700 links saved in 3.548 seconds, all financial fields unchanged, 2,700 decisions retained; a lost successful response preserves the draft and requires fresh records. A 50-finding package failed without producing an incomplete download, then retried to byte-identical contents; the 5,000-payment case survived refresh. Reader comparison across 58 sections preserves previous values/source cells and recovers 3 amounts, 19 balances and 11 directions. Four actual API sections match. No real-source financial writes. This is not independent extraction accuracy.

Private records remain ignored: daytime-identity-acceptance.json, daytime-recovery.json, daytime-compiled.json and daytime-andrews-spacing-comparison.json. Do not repeat their successful writers. Local API 58002 now runs in session 78838, logging to data/local-runtime/daytime-backend.log. Development UI 55174, compiled preview 55175, engine 58003/session 21252 and worker/session 5173 remain available. The compiled code check passed, but its baked local commit identifier predates code commit 2898f70e. The deployed identifier is verified separately below.

The branch was fetched with no incoming changes and 2898f70e pushed normally, followed by documentation checkpoint afae0841; local and remote match. The automatic deployment restarted successfully and the public version now reports afae084. All five public team-test assets match the checked local bytes. The result is retained in ignored data/local-runtime/daytime-deployed.json. The local plan now marks this verification complete. This documentation-only follow-up can join the next necessary checkpoint; do not trigger another deployment solely to record a deployment. Keep the evening schedule active after that; address actual feedback or demonstrated gaps without inventing work or repeating full suites. Team authenticated deployment acceptance and independent human extraction references remain open. Preserve unrelated untracked files and private PDFs.

## Larger statements, selections, graphs and reports, 16 September 2026

The four capacity changes requested before testing are implemented and locally verified. Single-statement review now allows 25,000 possible transactions and 100,000 retained rows, with 50 correction rows per page and compact change-only drafts. Named selections no longer stop at 100 payments. The graph no longer stops at 1,000 payments: repeated connections share an arrow and distinct connections are drawn 250 per page while every underlying payment remains accessible. Combined reports no longer stop at 20 findings or 20 supporting PDFs. Completed work remains checked in section 0 of the completion plan. The detailed record is [the 16 September capacity release note](releases/2026-09-16-financial-capacity.md).

Saving and packaging selected payments now reads their sources in ordered batches of 500 through a case-protected read-only endpoint. Missing or mismatched records refuse the entire save. The joined query avoids loading whole-statement metadata for each payment; the 2,700-payment selection saved in about 1.7 seconds locally. Saved selections and report data retain a 32 MB bound, original PDFs retain a combined 64 MB package bound, and complete graph preparation retains its 64 MB snapshot bound. The shared calculation row ceiling is 100,000. These are resource bounds, not performance measurements at every maximum.

An actual 167-page synthetic PDF produced 5,000 transactions with no flagged problems and imported once in about 15 seconds. Independent readback verified every amount, date, direction, running balance and original page. The existing three-account case retained all 2,700 payments through selection, refresh, save, reopening and graph paging. A 50-finding report containing that selection saved, reopened after refresh and downloaded with all three PDFs. Independent ZIP checks verified every payment, original file byte and report/member hash. Focused automated checks additionally covered the 25,000-transaction request boundary, 25,000 graph payments, 1,200-row edited review, 250 findings and 25 PDFs. No full suite was repeated.

Scoped lint and final TypeScript/production build passed; final bundle 13.22 seconds with the existing chunk warning. The compiled preview passed a final read-only browser check of the 5,000-payment case, saved 50-finding report and last graph page among 2,700 payments. No financial writes or JavaScript errors occurred. The guide was rebuilt with the new controls and bounds. User UI acceptance, independent accuracy measurements for real statements and deployed-build verification remain outstanding. No push or deployment occurred. Before a later explicitly requested push, inspect the remote and tree, then use the existing deploy process.

Private ignored records: data/local-runtime/single-statement-journey.json, single-statement-verified.json, capacity-journey.json, capacity-report-verified.json and capacity-compiled-verified.json. The 5,000-payment synthetic case is 399337ac-af00-4fd5-b490-4a95822fd196. The existing 2,700-payment case remains f017d904-3b74-48bd-8549-80deb184425e. Do not repeat their successful imports or saved reports. Real supplied PDFs were unchanged and were not imported by this task. Local UI ports remain 55174 and 55175; API 58002 runs in session 74677 with output redirected to data/local-runtime/capacity-backend.log. Engine session 21252 and worker session 5173 remain available. The old continuation heartbeat remains paused at its earlier cutoff.
...
Nine focused checks passed across the comparison form, comparison verification and saved response. The new component check was rerun after adding explicit unmount cleanup and passed without the earlier act warning. Scoped lint passed. The actual browser ran two read-only comparisons around one intercepted duplicate-decision failure; it verified the hidden old result, exact retained fields and fresh result, with all six current synthetic payments unchanged. No financial writes or JavaScript errors. Private claim-refresh-acceptance.json records the result.

The guide now explains the comparison reload action: 24,172 words and 27 synthetic images. TypeScript project checking and production build passed, with a 13.71-second bundle and the existing chunk warning. The compiled frontend passed sign-in back to the case, payment search/source/return, guide image loading, five-note report reopening/download and Findings refresh. All nine main-case payments were unchanged, with no failed assets or JavaScript errors. All fourteen local service/case readiness checks passed against the six-payment duplicate case. The remote fetch completed at about 23:26 Dublin: no incoming commits, 55 local commits ahead at 50c92646; remote head 40fdf650. No push. Next: record this local change and continue final review and incoming UI feedback through 23:59 Dublin under the existing five-minute heartbeat. Runtime remains API 83107, engine 21252, worker 5173, development UI 55174 and compiled preview 55175.

## Readable tracing account choices, 15 September 2026

Calculation refresh recovery is committed as 6ab3b251. The inspected tracing screenshot repeated the holder and account number because an existing display label was concatenated with those same fields. Account search and the selected-account caption now remove repeated label segments while retaining the original case, number, bank and currency. The display also cleans up an older saved label without changing its selected account ID.

Three existing account-filter checks and scoped lint passed. The actual browser selected the synthetic account, applied dates and refreshed with the same account and a single occurrence of each label segment. Six payments remained unchanged and no JavaScript errors occurred. Private account-label-acceptance.json retains the result; the tracing and transfer screenshots were visually checked and now replace the guide's older entry-control images. One helper retry corrected its attempt to find the transfer select's label as standalone text.

Production build passed in 13.21 seconds, including TypeScript project checking, with the existing chunk warning. The generated guide contains 24,118 words and 27 synthetic images; its published Markdown matches the source and contains no em dashes. Continue through 23:59 Dublin under the existing heartbeat, with no push unless newly requested. Runtime remains API 83107, engine 21252, worker 5173, development UI 55174 and compiled preview 55175. Next: inspect the final remote integration state after this local commit and incorporate any incoming UI feedback. Do not repeat successful imports, decisions, calculations or broad suites.

## Calculations require refreshed inputs after a case action, 15 September 2026

Excluded statement reopening is committed as ac1214bf. Manual transfer, pattern and tracing tools are retained across tab changes, so their mutation/state-held inputs did not participate in the normal query refresh after a financial decision. They now watch a shared, disabled case cache marker under financial-ledger. It makes no request and contains no financial data. After invalidation, each previously loaded tool independently hides its old calculation and provides the exact reload action. Loading the same payment set restores compatible drafts; a different set requires fresh review. Saved findings and reports remain unchanged. An invalidation while inputs are loading causes that response to be refused.

Sixteen focused checks passed across transfers, patterns, one-account and cross-account tracing, plus the shared guard. They verify retained explanation/opening assumptions, unrelated-case isolation, pending-response refusal and separate refresh state for two open tools. TypeScript and scoped lint passed. The actual browser loaded all four tools, intercepted one duplicate-decision POST with a simulated 503, then verified every reload notice, restored the single-account opening amount and retained the network dates. All six payments were compared unchanged, with no financial writes and no JavaScript errors. The one-account case correctly refuses a cross-account calculation; the earlier two-account calculation acceptance remains separate. Private analysis-refresh-acceptance.json retains the result, and its screenshot was inspected. Helper retries fixed a missing Find accounts step, an assumption that the one-account fixture had two accounts, and the expected uncertain-save message.

Guide updated to 24,118 words and 27 synthetic images. Transfer date/population/tolerance and pattern-window instructions now match the visible controls. Production build passed in 13.02 seconds, with the existing chunk warning. Runtime remains API 83107, engine 21252, worker 5173, UI 55174 and compiled preview 55175. Continue until 23:59 Dublin with the active five-minute heartbeat; no push without a new explicit request. Next: the inspected tracing screenshot exposes repeated account names/numbers in the older account search controls. Remove that duplication while preserving identification and selection.

## Excluded statement reopening and seven-source comparison, 15 September 2026

Duplicate refresh and read-only review is committed as 39faa523. A read-only API probe reproduced an excluded scanned copy returning current_import:null and appearing ready for a fresh import. Statement review now recognises the explicit exclusion throughout that PDF family, names the retained source, leaves its original visible and removes import/edit/reread invitations. The API refuses reimport. A legitimate replacement is recognised by its saved replacement request and remains available; restoring the duplicate decision restores its existing import route. Duplicate decisions invalidate the affected case's statement review cache too.

The first targeted backend run found a missing bank identifier in the new duplicate fixture and the need to distinguish replacement history from explicit copy exclusion. Both were fixed. Thirty-three distinct backend checks passed across that run and affected reruns, including restoration without a second import. Twenty-five frontend checks passed, with an existing act warning in the receipt test. TypeScript project checking and scoped lint passed. Actual headless browser verification reopened the excluded scanned PDF, decoded its source image, retained the exclusion after refresh and followed the decision link. Six current payments were compared unchanged; zero financial writes and no JavaScript errors. Private excluded-statement-reopen-probe.json and excluded-statement-reopen-acceptance.json retain the before/after evidence. Its synthetic banner screenshot was inspected.

The seven-source v15 capture finished successfully. Original sizes and hashes verified; 182 document sections retain all row fields, balances, account metadata, original source cells and flagged issues from the prior v13 capture. The 2,955 automatic candidate rows include unresolved entries and are not verified transaction counts. No financial records imported. Private completion-reader-v15-capture, reader-v13-v15-prediction-diff.json and reader-v15-field-comparison.json preserve this consistency comparison. Independent human references remain unavailable and unchecked.

Guide rebuilt: 23,935 words and 27 synthetic images. Production build passed in 13.54 seconds, with the existing chunk warning. API restarted as session 83107 with /tmp/loupe-excluded-reopen-api.log. Engine 21252 and worker 5173 remain active; development UI 55174 and compiled preview 55175 remain available. Continue through 23:59 Dublin under the active five-minute heartbeat. No push without a new explicit request. Next: inspect any remaining concrete end-to-end failure or incoming UI feedback; do not repeat successful writers or broad suites.

## Duplicate details refresh and read-only review, 15 September 2026

The complete duplicate import/exclusion/restoration fix is committed as 085ac28d. A follow-on regression reproduced the previously opened payment-source cache remaining fresh for two minutes after a duplicate decision. Exclusion and restoration now invalidate only the affected case's payment sources, related analysis payments, verification summary and statement-import status, alongside its transaction and decision lists. A failed or uncertain decision also refreshes those records. Other cases' caches are unchanged.

Duplicate cards now state Included in Transactions or Excluded from Transactions, name the retained filename and keep internal identifiers behind an expandable detail. Read-only members keep comparison and source controls but do not see exclusion/restore buttons leading to an unavailable form. Twelve focused frontend checks passed, including the previously failing cache check and editable/read-only controls. Real TypeScript checking, scoped lint and a production build passed (15.31 seconds, existing chunk warning). An actual temporary read-only member opened both native and excluded scanned originals, saw current inclusion labels and made no financial writes; the member was removed. Private duplicate-readonly-acceptance.json and its inspected screenshot retain the result. Guide updated to 23,841 words and 27 synthetic images.

The seven-source capture finished and its v15 comparison is recorded in the later checkpoint above. Preserve both captures; neither is an independent human accuracy benchmark.

## Confirmed PDF duplicates can be excluded and restored, 15 September 2026

Flagged-row correction navigation is committed as d6304260. The next actual browser journey imported the native and scanned synthetic statements once each and opened their six exact source payments. Both copies matched, but exclusion first failed because the retained P3 import was rejected by an old verified-only rule. After that fix, the actual PostgreSQL constraint exposed a second defect: a newly imported document had no persisted duplicate-group key. These were genuine failures, not deployment blockers.

The confirmed decision now accepts admitted PDF copies in working totals, preserves each verified payment occurrence by account identity, currency and content, and keeps all existing revision, case, held-row and correction checks. It records fresh duplicate fingerprints atomically with the exclusion and its before/after history. Preview remains read-only. Neither source nor row proof classes are promoted. Each candidate now has View source, including an excluded copy, and the introductory instructions explain what to do.

Thirty-seven distinct focused backend checks passed across decision, query and router checks, including new imported documents without stored fingerprints, exact reversal, mixed verification, repeated payment occurrences and held rows. Nineteen focused frontend checks passed; scoped lint, real TypeScript checking and production build passed (13.24-second bundle, existing chunk warning). Test helper corrections were required for a missing quarantine reason, duplicate fixture hashes forbidden by the database, a Testing Library-only option and the protected image blob URL. Rebuilding guide assets interrupted one development-login check; that helper was stopped and rerun with frontend files stable. No broad suite repeated.

The actual browser imported twelve payments, excluded six, opened both original PDFs, restored the copy to twelve, then excluded it again to six. Final money in USD 1,550.00; money out USD 895.00. All six current payments remain P3, and verified totals contain zero. The downloaded package independently passed: six current table rows, all twelve stored readings, three exact exclusion/restoration events, both original PDF hashes, and all readings in its HTML. No JavaScript errors. Private records: scan-import-duplicate-acceptance.json, duplicate-source-restoration.json, duplicate-export-verified.json. Do not repeat those writers. Both PDFs are synthetic in case ffe260f2-c300-4d5e-83bc-3fcca19cc212; no real statements were imported.

The guide now gives exact duplicate comparison, source, exclusion and reversal steps: 23,720 words and 26 synthetic images. Runtime API session 46389, /tmp/loupe-duplicate-copy-backend-v2.log; engine 21252, worker 5173, development UI 55174 and compiled preview 55175 remain available. Continue concrete UI/workflow checks and incoming feedback through 23:59 Dublin. The five-minute heartbeat remains active. No push without an explicit request.

## Flagged statement rows open their correction controls, 15 September 2026

Scanned-column interpretation is committed as8b220ba7. Rows needing a layout check now offer Review this row when the user can edit an unimported statement. It opens the correction controls, focuses that row's Use checkbox and shows its original location. Original text remains separately clickable. It does not confirm, exclude or write anything automatically; existing imported statements keep their transaction-edit route.

Twenty-two focused statement/table checks, scoped lint and the production build passed (12.49-second bundle, existing chunk warning). The actual browser opened the synthetic scanned footer, excluded it with a specific reason, refreshed, reopened the same row and retained both the explanation and exclusion. The confirmation was enabled for exactly six payments. All eight original table rows stayed visible, the original footer location was highlighted and zero financial rows were imported. Screenshot inspected; no JavaScript errors. Private flagged-row-acceptance.json retains the result. Guide rebuilt:23,573 words and26 synthetic images.

The earlier compiled frontend check used the deployment's npm run preview command on loopback55175. It passed sign-in back to the case, payment/source navigation with a retained search, guide text/image loading, reopening/downloading the saved five-note report and Findings refresh. Nine bank payments unchanged, no failed assets or JavaScript errors. Private compiled-frontend-acceptance.json records that check. Preview session5306 remains running; it serves the newly rebuilt dist. Normal development UI remains55174. API session83254; engine21252; worker5173. No broad suite or push.

Next: finish the new scan's confirmed-import journey, then inspect how its distinct-byte native copy is compared as a possible duplicate. Use the dedicated synthetic no-AI case and guard any new confirmation/decision by saved IDs. Do not repeat the original main-case writers or import real evidence. Continue the five-minute heartbeat and incoming UI feedback through23:59 Dublin.

## Scanned credit/debit columns corrected, 15 September 2026

Worker startup without AI credentials is committed as7306b43c. Its actual scanned-PDF check exposed the midpoint-column assumption: left-aligned printed money headings and right-aligned values crossed those guessed boundaries. Generic statement proposals now compare the left, middle and right boundaries between adjacent money headings across the current printed section. They retain only layouts supported by the most positioned date/money rows; disagreements remain explicit layout exceptions. A repeated printed header starts a separate comparison. No amounts are guessed from running-balance arithmetic, and original source indices/text/rectangles remain unchanged. Proposal revision is statement-review-v15 so existing drafts must match the current interpretation before import.

The printed table uses the reader's source-column references when available, retaining the original displayed text and blank credit/debit cells even when numeric text extends beyond its heading. Remaining unmatched text stays visible. Forty-eight focused backend proposal/import checks passed. The first ambiguity check exposed that alignment alternatives must apply only between monetary columns, leaving the independently aligned date/description bands fixed; this was corrected before the passing run. Twenty-seven frontend layout/import/table checks, type checking and scoped lint passed. The production build passed (12.86-second bundle, existing chunk warning). Actual browser acceptance compares all six native/scanned payment dates, amounts, credit/debit directions and balances, verifies blank cells and both controls, then opens the original scanned balance image. The screenshot was inspected. The synthetic footer is still one explicit unresolved line; this was not hidden to claim complete automatic extraction. Zero financial rows imported and no JavaScript errors. Private scanned-columns-acceptance.json and screenshots retain the result.

The engine/worker remain on their no-key sessions21252/5173. API restarted as session83254 using /tmp/loupe-scanned-columns-backend.log. The guide now explains scanned-column handling. No broad suite, real-evidence mutation or push. Continue final deployment/user-flow review and incoming feedback through23:59 Dublin with the existing heartbeat.
...
A restored selected account name is retained when searching other account names. A failed lookup keeps the chosen account and dates, and Try loading accounts again repeats the current search. Nineteen focused checks passed (17 ledger panel, two account chooser), with TypeScript project checking, scoped lint and the final production build (7.03-second bundle, existing chunk warning). Actual browser checks verified statement-to-account expansion, readable populated/empty scope, date/selection restoration after refresh, and failed account-search recovery. A helper initially waited for a native select option to become visible; it was corrected to read the selected option's attached text. Nine payments are unchanged, and the scope journey reported no JavaScript errors. Private account-scope-acceptance.json and account-chooser-acceptance.json retain results.

Guide updated with account-wide opening and search retry instructions. No broad suite, real financial write or push. Runtime unchanged. Next: inspect the remaining original-file and saved-report entry points for actionable empty/error states, reusing the existing synthetic case and avoiding repeated broad verification. Keep the five-minute schedule active through 23:59 Dublin and incorporate incoming UI feedback without asking to continue.

## Analysis selections stay with the case, 15 September 2026

Selection correction recovery is committed as 2609cf03. A real browser probe confirmed that LinkedPayments discarded selected IDs when its list was hidden, and they were absent when returning to Transactions. Linked analysis lists now use the same user/case selected-payment draft as Transactions. Their counts explicitly include selections outside the current result. The shared selection review can inspect or remove those choices without losing the remaining work. Saving retains the whole named selection. Source reads use the same case/ID/status checks and cancellation signal; read-only analysis lists no longer offer Add note, and page controls lock while loading.

Six focused linked-list checks passed, including closure/reopening, sharing selections from another result and case/user isolation. TypeScript project build, scoped lint and npm run build passed (12.61-second bundling, existing chunk warning). Actual browser acceptance retained the counterparty choice after hiding/reopening its list, carried it into Transactions, added a second payment, refreshed with both retained, then removed one through analysis review and confirmed the change in Transactions. All nine bank payments remained unchanged. Private analysis-selection-probe.json and analysis-selection-acceptance.json retain results. Guide rebuilt: 23,291 words, 26 synthetic images.

Next: replace the confirmed opaque account ID in the applied transaction scope with the recognised account label, including empty search results. The actual imported-statement check exposed this remaining visible internal ID even though the account chooser and rows have readable names. Keep the change limited to account identification and accurate scope wording. Continue incoming UI feedback through tonight; five-minute heartbeat active, no push. Runtime unchanged.

## Selected-payment correction recovery verified, 15 September 2026

Imported-statement account/source navigation is committed as 619286bb. A real browser probe reproduced an old selected payment becoming unsaveable after correction, with a generic error and no individual removal control. Review selected payments now lists the chosen payments across accounts and filters, twenty at a time, with date, amount/direction, account, original filename and current status. Open transaction retains original/corrected source navigation. Remove from selection changes only that draft ID; it does not alter evidence or saved findings. A failed load remains distinct from an excluded payment and can be retried. Source responses must match the selected ID, case and correction status before displaying or saving.

Eighteen distinct focused checks passed across selected-payment review, saving and the surrounding table. The first review test run had an accidentally returned mock cleanup function and a loading-status timing assumption; those test helper errors were fixed and its five checks rerun. The actual TypeScript project-reference build (tsc -b) passed, as did scoped lint. The earlier bare tsc --noEmit command was a no-op for this project-reference root and is not verification; use npm run typecheck or tsc -b from frontend_v2 going forward. The app bundle built successfully, retaining the existing chunk warning.

Actual browser acceptance named the changed synthetic payment, opened both original and corrected values, removed only the old selection, retained the remaining ID and note through refresh, then saved and reopened one finding. A subsequent read-only check verified the exact saved payment ID and temporary payment-check failure/retry without losing selections. Nine bank payments are unchanged; no JavaScript errors. New synthetic finding b1209192-693e-458d-80ad-da50b64f1c96 in case 95081ef8-8ffe-475a-bc32-fc77aa1bd0e6 must not be saved again. Private selection-recovery-probe.json and selection-recovery-acceptance.json retain results. Guide adds a synthetic selection-review image and matching steps (23,222 words, 26 images).

Next: inspect selection from linked analysis results. LinkedPayments currently stores its own chosen IDs only in component state, so check whether closing its list or returning to Transactions loses them. Fix only a demonstrated continuation defect. Keep working through tonight with the active five-minute heartbeat; no push or real-evidence writes. Runtime unchanged.

## Imported statements open their own payments, 15 September 2026

Other-record view recovery is committed as c51a9311. A local browser probe reproduced Open imported transactions leaving the previous savings account selected when opening a checking statement. Existing-import receipts now include their saved account and filename. Opening or confirming an import applies that account, clears stale dates/table filters and visibly filters by the exact imported source. The named statement filter survives refresh and can be cleared. Explicit account navigation starts a fresh table view; separately selected payments and other drafts remain intact.

Twenty-seven focused frontend checks and nine backend checks passed, with TypeScript, scoped lint and production build (12.76 seconds, existing chunk warning). Actual browser acceptance switched from savings to the checking statement, cleared its old search, showed exactly six statement payments and retained the prior selection through refresh. The checking account contains seven payments across two sources, so the helper's first assumption of six account-wide payments was corrected against the API. All nine case payments remained unchanged; no JavaScript errors. The downloaded table records the source filter, six exact IDs including the saved USD 125 correction, and the readable filename. Its wider snapshot retains eight readings including history. Private open-import-scope-acceptance.json and open-import-table-export-verification.json retain results. Guide rebuilt: 23,053 words, 25 synthetic images.

Remote fetched: 42 local commits ahead, zero behind origin/integration/evidence-main-reunion before this batch; remote 40fdf650. No merge needed and no push. API session 92585, PID 76449, /tmp/loupe-import-navigation-api.log; other services unchanged. No broad suite or real-evidence writes.

Next: check a selected payment that is corrected or excluded before its named selection is saved. Ensure any unavailable selection can be identified and removed without clearing valid selected work. Continue actual workflows and incoming UI feedback through tonight, with the five-minute heartbeat active until 23:59 Dublin. No push without an explicit request.

## Other-records view and original-file return verified, 15 September 2026

Saved-work navigation is committed as f358a5a9. A local browser probe found that refresh changed Other financial records back to imported payments and cleared its search and amount filters. The dataset now remains in the case URL. Other-record search, categories, person, dates, amount bounds, ordering, table page and expanded/selected record keys are stored as user/case browser-tab drafts. Sender/receiver comparison selections also use case/user drafts. Moving to another case restores that case's own view. Choosing the already-active dataset no longer clears the current page or expanded records. If fewer results remain, table contents and pagination both use the last available page without overwriting a temporarily unloaded selection.

The source round trip exposed a missing case ID in the other-record original viewer, so it now receives the case through the table and displays Open file location. The completed browser check opened original PDF page 3, visited its exact Evidence file, returned and refreshed with filters, sorting and the expanded record retained. Two cases kept separate filters; Reset all survived refresh; Trends retained the other-record dataset. Five and six other-evidence records and nine bank payments were compared unchanged, with no JavaScript errors. A helper initially compared the whole table including transient PDF-loading captions; its final comparison uses only financial rows. Private other-record-view-probe.json and other-record-view-acceptance.json retain the initial and final outcomes.

Fifty-six distinct focused checks passed: 50 view/navigation/pagination/page checks, five store checks and one changed source-detail check. TypeScript, scoped lint and production build passed (13.77 seconds, existing chunk warning). Guide rebuilt: 22,924 words, 25 synthetic images. Runtime unchanged. No broad suite, real-evidence writes or push.

Next: fetch the remote branch to check whether the prepared local work needs incoming integration, then continue remaining actual workflow checks and incoming UI feedback through tonight. Preserve unrelated files and make no deployment without an explicit push request. Keep the existing five-minute heartbeat active until 23:59 Dublin.

## Saved findings navigation and retry verified, 15 September 2026

Payment-table persistence is committed as 51e5b9d0. An actual local browser probe confirmed that following Edit or review in Workspace from a finding and going Back returned to Transactions and lost the finding search, although the report selection survived. Financial tabs now remain in the URL. Refresh and browser Back restore the requested tab; a fresh case URL still opens Transactions. The mounted page subscribes to tab changes so file-list actions use the same route. Findings search/page and saved-report pagination use the existing user/case browser-tab draft store. Failed loads offer explicit retry controls, retaining the same search/page and report selection. Pending pages cannot be skipped or selected as if they were the new results.

Fifty-one focused checks passed across nine new navigation/list checks, thirty-eight surrounding FinancialPage checks and four store checks. The run exposed duplicate sibling React keys on Tabs and the correction dialog; they are now distinct, and two affected page checks passed again without that warning. TypeScript, scoped lint and production build passed (7.38 seconds, existing chunk warning). Actual local browser acceptance retained Findings/search/selection after Workspace, Back and refresh, retried simulated finding/report load failures and confirmed all saved casework and nine payments unchanged. No JavaScript errors. Private findings-return-probe.json records the original failures and findings-return-acceptance.json records the completed check. Guide rebuilt: 22,857 words, 25 synthetic images.

Runtime unchanged; no broad suite, real-evidence changes or push. Next: check whether switching to Other financial records also survives original-file navigation and refresh, and inspect its current filter state across cases. Continue useful workflows and incoming UI feedback through tonight with the existing active five-minute heartbeat.

## Payment table position survives original-file navigation, 15 September 2026

Source preview recovery is committed as b78f2fc2. A real browser check found that opening a payment's original file, following Open file location to Evidence and going Back lost the payment search. Payment table settings now use the existing user/case-scoped browser-tab draft store, with separate settings for the applied account, date range, status and investigation view. Search, currency, amount bounds, money direction, review filter, sort order and page survive route changes and refresh. Changing a filter starts at the first matching page; Clear payment filters persists the reset without clearing the separately retained payment selection.

Seven focused table checks passed. The isolation test was rerun after wrapping its auth-store update in act to remove a test warning. TypeScript, scoped lint and the production build passed (7.40 seconds, existing chunk warning). Actual browser checks retained search and selection in the nine-payment case. The 2,700-payment case retained USD, newest-first ordering, its second table page and selected payment through separate account scopes, original-file navigation, Evidence, Back and refresh. A new reference search returned its single expected payment on the first page. Both complete payment lists are unchanged; no JavaScript errors. Private financial-evidence-return.json and volume-view-persistence.json retain the results. Guide: 22,736 words, 25 synthetic images.

No broad suite, real-evidence writes or push. Runtime unchanged. Next: check reopening saved findings and their linked casework, including failures and returning to the same saved-work view. Continue useful workflow checks and incoming feedback through tonight, with the active five-minute heartbeat.

## Source preview cancellation and file-opening recovery verified, 15 September 2026

Missing-amount corrections are committed as f0c172ed. Four focused delayed-response checks reproduced old source bytes being delivered after cancellation/sign-out, an older page replacing a reopened preview, and the same URL staying visible after the signed-in session changed. The shared protected-file reader now checks cancellation and session after reading bytes. Its object URL hook keys results by the signed-in token, ignores cancelled/stale responses and releases obsolete URLs. Existing original-source highlights continue working.

A failed Open in new tab request now leaves the current PDF visible with Try opening again. Pending repeat opens are locked, and a late failure cannot attach to a different file. Eighteen distinct focused checks passed across the helper, source highlight and viewer groups; scoped lint, TypeScript and production build passed (7.36 seconds, existing chunk warning). The real local browser retained its page during a simulated 503, retried successfully, downloaded bytes matching the source SHA-256, closed back to transaction details and left all nine payments unchanged. The headless browser downloads the PDF from the new tab; two helper attempts initially waited for a native PDF navigation event, then the actual download was captured and verified. No claim of native headless PDF rendering. The inline failure screenshot was inspected. Guide: 22,674 words, 25 synthetic images.

Private source-open-recovery.json and source-open-retry-original.pdf retain exact evidence and digest. API session 73330 and other services unchanged. No broad suite, real-evidence writes or deployment. Next: verify Open file location into Evidence and browser-back return to Financial, including the current search and selected payments. Continue through tonight and retain the active five-minute heartbeat without asking to resume. No push.

## Missing financial amounts can be corrected, 15 September 2026

Original-file navigation is committed as 66a19bda. Explicit financial records with absent amounts now remain visible. Their null original value is distinct from an omitted correction guard, checked before a file correction and while the database write lock is held. A later correction preserves the original absence rather than replacing it with the first entered amount. In mixed older/newer cases, an older record no longer discards another record's explicit financial classification. Ordinary nodes with no financial classification or amount remain outside the list.

Fifty focused backend checks and thirty-three frontend checks passed, with TypeScript, scoped lint and production build (7.06 seconds, existing chunk warning). A rolled-back local Neo4j check verified legacy payments, tagged payments, tagged claims, missing amounts and ordinary nonfinancial nodes in the correct sets. Actual browser checks showed both missing values, restored a failed save through refresh, saved one individual and one correction-file edit, retained original absence after a second explained edit, refused a stale request with 409 and left all nine bank payments unchanged. The five-record report retains both exact new keys, original absence and latest reasons, totals USD 465.00, and has two readable pages with repeated table headings. Both pages were inspected. Guide rebuilt: 22,635 words, 25 synthetic images.

Synthetic missing-amount records cffb9d02-978a-5261-92b0-f471c8bb29f9 and 3da56079-db2c-5770-a8e3-c82d034d7ee1 are in case 95081ef8-8ffe-475a-bc32-fc77aa1bd0e6. Their final values are USD 125 and USD 50; the first second-edit reason explicitly identifies the synthetic change from the receipt's 120. Do not repeat these edits. Private missing-amount-fixture.json, missing-amount-report-verification.json and before/after PDFs retain the check. API session 73330, PID 72366, /tmp/loupe-missing-amount-api.log. Other services unchanged.

Next: examine the shared authenticated image/blob hook used by original-source highlights when a page request finishes after closing and reopening the same source. The existing hook lacks a post-read cancellation check and does not key its result by the signed-in session. Reproduce with focused delayed-response checks before changing it. Continue useful workflow checks and incoming UI feedback through tonight; no broad suite or push. Keep the existing five-minute heartbeat active.
...
Synthetic case 95081ef8-8ffe-475a-bc32-fc77aa1bd0e6; corrected other-evidence records 51909820-70db-5895-86fe-92b517882343 and f28adff9-dae1-5008-922a-ce4f82bdea67. Do not repeat those corrections. Private unknown-amount-fixture.json, unknown-amount-report-verification.json, unknown-amount-final-download.json and unknown-amount-after-downloaded.pdf retain the check. Guide: 22,524 words, 25 synthetic images. No real evidence, broad suite or deployment changes. API session 83702, /tmp/loupe-unknown-amount-final-api.log; engine/worker remain running.

Next: verify the shared original-file viewer from imported payments and statement sources, including returning to the same investigation view. Continue actual user workflow checks and incoming UI feedback through tonight; keep the existing five-minute heartbeat active until 23:59 Dublin. No push without an explicit request.

## Original files, recorded dates and sign-in recovery verified, 15 September 2026

Continue without stopping or pushing. A temporary profile request failure now keeps the sign-in while hiding private pages until Retry connection succeeds. Expired/refused sessions still sign out. Logging in returns to the exact requested internal case route. Thirteen focused auth checks and real browser temporary-failure/expiry/return checks passed.
...
Private evidence: source-date-ui.json, source-date-report-verification.json, source-date-fixture.json and source-date-report.pdf. API session 34362 uses /tmp/loupe-pdf-page-api.log. Existing engine and worker continue. The five-minute heartbeat remains active until 23:59 Dublin.

Next: a read-only service check confirmed that an unreadable amount such as "not stated" becomes numeric zero in Other financial records. Fix this representation and its applicable correction/export paths without changing imported bank payments or repeating completed broad checks. Continue incoming UI feedback and workflow checks through tonight.

## Source custody recovery verified, 15 September 2026
...
Synthetic case 95081ef8-8ffe-475a-bc32-fc77aa1bd0e6, file 6968cd25-924a-4013-a62c-138ebe6c986a. Receipt 1e003ee3-7bdd-4c47-a519-ee4a16b4ca31, correction a2bb7969-7ed2-4c45-a6cb-f8e89356b7a6. Do not repeat these saves. Private custody-recovery-ui.json and custody-export-verification.json retain the evidence; custody-ledger-export.zip retains the output.

Next: fix the reachable profile-loading catch that logs out on any connection failure, while retaining strict expired-session handling. The redundant browser navigation exposed that path. Then finish the other-evidence expanded-record view: no assumed USD, readable recorded metadata and no inactive editing/selection controls. Keep working and leave the five-minute heartbeat active through tonight. No push without a new explicit request.

## Categories, summaries and charts verified, 15 September 2026
...
Next: Source custody records still use unretained form fields and a memory-only submission ID. Preserve that draft and the exact retry identity through closing the source panel and refresh, then verify the original history is unchanged. Keep the five-minute heartbeat active through tonight. API session 66345 on 58002 has the final report/category code; engine session 11645 and worker session 87758 remain running.

## Evidence report corrected and checked, 15 September 2026

Continue without pausing or pushing. Evidence corrections committed locally as fe0b3d2d. The Other financial records report now retains amount signs, identifies each currency and groups totals separately. Missing currencies or amounts are not totalled. Original corrected amounts and latest explanations are included. Automatic dollar signs, inferred money-in/out labels and an unsupported default privilege stamp were removed. Report downloads carry the current case name and all supported filters, including amount limits and category/entity names containing commas. The browser shows pending/error states, permits retry, supports the printable-HTML fallback and discards stale case/session responses. Downloads are bounded to 64 MB.
...
Next: finish the reachable Categories dialog failed-save/recovery path, including category assignment feedback on other evidence. Keep transaction grouping separate from claims. Continue the existing five-minute heartbeat through tonight and incoming UI feedback. Local API session 34474, /tmp/loupe-evidence-report-api.log, needs one restart to load the final compact PDF style. Do not repeat the broad suites.

## Evidence amount corrections verified, 15 September 2026

Continue without pausing or pushing. Correction files now use exact record keys, reject invalid or rounded amounts and display per-record saved/failed results. A downloadable CSV template supplies the current list's keys and amounts. Invalid replacement files clear older previews, overlapping reads cannot replace newer files and pending saves are locked. Expected-amount checks refuse stale previews before writes and protect each database update with a write lock. Individual amount correction is reachable from Other financial records, uses the recorded currency and retains the original amount and latest explanation. Read-only users cannot invoke these edits.
...
Next: inspect the remaining secondary evidence editors for pending-save/error handling. Do not enable transaction-only grouping on claims merely to exercise unused controls. Continue authorised user workflows and incoming UI feedback through tonight. Current local API 59753, session 5561, /tmp/loupe-correction-final-api.log; restart once to load the last numeric guard adjustment. No broad suite repeat.

## Advanced review recovery and claim presentation verified, 15 September 2026

Continue without pausing. Decision/evidence correction batch is committed locally as 4be283c2. The next batch retains advanced reading edits per exact case/file/mapping/candidate/review version. Successful saves clear the draft while showing the submitted fields until reload; unsuccessful saves retain it. Provisional-account label/reason stay separate by currency and review revision. Finalization retains its explanation, resets both confirmations after reload and requires explicit review before reusing a reason against changed preview rows or statement controls. Existing server-saved statement-control drafts remain unchanged.

All 34 affected advanced-review tests passed. Browser candidate-draft-recovery.json verifies amount, currency, separate booking/value/transaction dates, counterparty, reason and provisional account after refresh. Existing synthetic case 61272494-00de-4a97-8ba9-77fce4be2f24, mapping d8ff318d-8112-488e-bfbd-0fbbf0712ef0, candidate 3d2d3475-11cd-44f2-824c-2e44d6a8b9a4. No account/review/financial writes or JS errors. Original source image available.

Claim comparison presentation puts matches first without dropping any compared payment. Supporting choices show dates/amounts/descriptions. Per-field comparisons expand under Why this result. Eight focused checks passed; claim-presentation.json verifies all seven payments retained and USD 900 first. No second saved note. Type checking/scoped lint and production build passed (7.17 seconds, existing chunk warning). Rebuild and commit guide with this batch. Continue checking the remaining interactive financial actions and incoming UI feedback; keep the five-minute heartbeat active through tonight. Do not push without a new instruction or repeat the broad suites.

## Continuing advanced review after decision recovery, 15 September 2026

Keep working without asking to resume and leave the five-minute heartbeat active through tonight. Identity recovery is committed as 0fa9dd0d. The next verified batch persists claim form inputs separately by account, review decisions by exact claim inputs and calculation hash, duplicate reasons by both document revisions, and row-exclusion reasons by payment/status. Changed claim results and missing selected payments require explicit review. The other-evidence AmountEditDialog now initializes the actual selected amount, respects currency, preserves drafts by record/source value, blocks repeat Enter/close while saving, and clears only after success. Unknown original values remain unknown. Claim screen and saved-note wording now give direct instructions.

Focused verification: 42 decision/claim checks, five amount-editor checks and 38 FinancialPage checks passed, plus type checking and scoped lint. The current guide is rebuilt with 23 synthetic images. The real browser restored every claim field, response and selected payment after refresh and recalculation; original source image and comparison download hash passed. Exactly one note, 917c963e-f40c-4b0f-a79a-86e93b6b1ec5, saved in synthetic case 95081ef8-8ffe-475a-bc32-fc77aa1bd0e6. Private claim-recovery.json, claim-review-saved.json and claim-comparison-acceptance.json retain the result. Do not repeat that save. The initial helper waited on the wrong endpoint after success; read-back verified the single existing note. No application failure or duplicate note resulted.

Continue CandidateReviewForm, CandidateAccountForm and finalization explanation recovery next. Individual candidate fields currently disappear before Save review; statement controls already have explicit server draft panels and need no duplicate mechanism. Keep finalization confirmations fresh after a new preview. No broad suite repeat. No push without a new request. This is a progress checkpoint, not a stopping point.

## Continuing identity recovery and decision review, 15 September 2026

Keep the five-minute schedule active through tonight and continue without asking to resume. Graph and selection refinements were committed locally as 37916767. The next actual browser check reproduced lost payment-name selections, name, search and explanation after refresh. Payment-name and account-owner forms now retain user/case-scoped browser drafts, including the revision originally reviewed. Changed records require Use latest payment/account records before a new save; missing current payments remain explicit until removed. Reload retains drafts. A successful account-link response must contain the requested links, not just applied=true.

Browser acceptance passed: three selected payments survived refresh and a simulated failed save, then saved exactly three decisions. Party 4020d9f0-e78b-4c0d-ba9a-d457e141db07 combines EUR 4.14 and USD 2.07 separately, with all 2,700 payments and original values unchanged. The analysis snapshot hash and three decision IDs were checked. Two account selections and explanation also survived refresh; owner 3e316432-75d2-4019-a0ab-274753cce810 saved exactly two links with unchanged printed names/identifiers. Its January 1-3 account group contains 176 payments, EUR 554.70 incoming and EUR 1,079.54 outgoing. Both are in the existing synthetic volume case. Private volume-identity.json, volume-account-links.json and identity-draft-before/after.json retain the evidence. Do not repeat these saves.

Seven payment-directory and five account-directory tests passed, including revision changes, unconfirmed saves and unavailable selected payments. Type checking and scoped lint passed. Guide updated and rebuilt with 23 synthetic images. Commit these recovery changes locally with detailed explanation. Nothing pushed. Continue the remaining decision forms: RowAdjudicationDialog, AmountEditDialog, ClaimComparisonDecision and duplicate/candidate review. Inspect their actual calling paths and existing explicit draft controls before changing them. StatementScopeEditor already uses StatementEditorDraftPanel for explicit saved drafts; do not mistake its local editing state for absent persistence. No further broad suite at this point.

## Continuing large-case investigation, 15 September 2026

Keep working and leave the five-minute schedule active. The four volume fixes are committed locally as 65515d9a, 24c2cf2b, 610be31a and e67ad123. Final API/engine/worker code was restarted; the refreshed whole-case archive independently verifies all 2,700 payments and three original PDFs. No push.

The next actual workflow saved 100 selected payments across three accounts and table pages. Selections survived account changes, financial tabs and refresh. Saving took five seconds. Finding bc004765-0829-4bab-8f53-c48daa7e9942 and report 63f2b3b6-f7bc-420c-b00c-77ffedfd1f94 reopen with all 100 exact values, three account/currency totals and three unchanged PDFs. Private volume-findings.json and volume-findings-report.zip retain the evidence. Do not repeat these saves.

Uncommitted UI refinements now explain the 100-payment limit and disable additional checkboxes until one is deselected. A large graph revealed that selecting a name filtered only the list, leaving all connections drawn. It now focuses the graph too, with a name search, visible selected/full count and Show all connections. Dense graphs hide small labels until zoomed. Graph names use the recorded name without the internal Source label prefix; graph/timeline bounds explain the account/date action plainly. All 2,700 monthly trend totals and all 62 same-day EUR 50 threshold groups match the generator. The 900-payment graph response took 1.2 seconds; selected source and graph render passed. No JavaScript errors. Private volume-analysis.json and captures retain results.

Seven selection/table tests, four graph tests, two graph source/boundary checks, type checking and scoped lint passed. Guide updated with the actual controls and a 23rd synthetic image. Record these verified refinements in a detailed local commit. Next useful work: check linking selected payments to a person/business in the larger case, regrouped totals, persisted decisions and original references; handle incoming user UI feedback promptly. Do not rerun the completed broad suites or pause at this commit.

## Synthetic volume acceptance complete locally, 15 September 2026

Continue work after recording these commits. Do not pause the five-minute heartbeat or push without a new request. All 2,700 synthetic payments now import across three accounts and two currencies. The exact comparison validates every date, amount, direction, running balance, account, currency and PDF page. The table opened in 1.8 seconds; 50-row pagination, currency totals, account/date filtering to 87 rows and final-payment source page 31 passed without JavaScript errors. Whole-case export took 1.6 seconds. Its verified ZIP retains an 18 MB snapshot, all 2,700 HTML rows and three byte-identical PDFs. The compact report was visually checked and has no page overflow. Private volume-journey.json, volume-investigation.json and volume-export-verified.json retain IDs/results.

Completed fixes below are ready for detailed local commits: 1,000 possible payments plus retained headings within a 10,000-row review envelope; reader v14 page-counter handling; full large-draft recovery; separate PDF worker processes with progress/error/cancellation and real OCR; source-summary query avoids repeated source metadata; a 64 MiB ledger snapshot inside a bounded 128 MiB archive with matching verifier limits; compact readable large-case reports retaining complete JSON originals. Focused 44 statement, six draft, 47 engine plus six final worker, 65 summary/snapshot, 39 export and two final report/boundary checks passed. Groups overlap. Scoped lint and production build passed; existing chunk-size warning only. No full suite repeated.

After committing, restart local API/engine/worker for final loaded code, refresh the saved synthetic export and continue selection/finding/report workflows on the larger case and incoming user feedback. Keep all prior real PDF imports untouched. Nothing pushed.

## Active synthetic volume fixes, 15 September 2026

Session recovery is committed locally as 9a3922c2. Continue without pausing. The 2,700-payment acceptance case f017d904-3b74-48bd-8549-80deb184425e exposed three real defects: the 1,000-row review limit counted metadata, generic Page N of M footers became unresolved payments, and batch PDF reading ran unsupported concurrent PyMuPDF threads. New limits retain up to 10,000 complete review rows but cap possible/imported payments at 1,000, including unknown rows. Draft storage accepts the complete review. Reader v14 excludes exact standalone page counters only with a transaction table. All 44 affected backend statement checks and six draft checks pass.

PDF jobs now run in separate spawned processes with bounded concurrency, progress, failure and cancellation cleanup. Existing OCR/geometry unit tests use an explicit inline-worker fixture for their test doubles; separate real-worker tests verify concurrent cell identity, configured limits, crash and cancellation. All 47 affected engine checks pass. Actual three-file parallel rereading retains all 62 source tables per file, whereas the threaded run lost a table and the original batch damaged one PDF first page. Old readings retained. B/C synthetic PDFs have new versions recorded in data/local-runtime/volume-journey.json. A's 900 transactions imported; the resumable browser importer is currently adding B/C.

A further volume slowdown came from joining full source-document metadata onto every payment in ledger summaries. Source load now selects only needed citation/disposition columns in the same statement. 65 summary/snapshot checks pass, including a large source payload that must not be selected repeatedly. Updated local API and engine/worker are running. Continue the volume import, investigation pagination/search/account/date/currency/source and complete export checks. Scripts and private evidence stay ignored in data/local-runtime or /tmp. No full suite repeat or push. Heartbeat stays active through tonight.

## Active session recovery completion, 15 September 2026

Navigation/search committed locally as d9fee2cc. Uncommitted session fixes now clear React Query case data on login/logout and identity changes, ignore stale profile responses, and prevent a delayed 401 from deleting a newer token. Financial permission failures distinguish expired session from denied case access, hide case contents and offer Sign in again/Open cases/retry. Focused 22 checks passed after reproducing all three stale-cache failures; production build passed. Real temporary-member revocation/restoration and simulated 401/sign-in recovery passed with no financial mutations or JavaScript errors. Private session-ui-check.json; member deleted. Scoped lint and guide updates passed. Commit this session recovery work locally, then continue a larger synthetic investigation case for pagination/totals/filter/source behaviour. Five-minute schedule stays ACTIVE through tonight; do not pause at a commit or push without new instruction.

## Active large-statement recovery work, 15 September 2026

Continue the five-minute schedule through tonight. Access changes committed locally as 1c1fb397; no push. The next real workflow check found a refresh gap: draft values survived but the selected file/period/page did not. Navigation now persists in sessionStorage by user/case/file and reading revision. Added a searchable section list with PDF ordering, match counts and clear search. Private large-review-check.json proves a correction draft stayed separate across two periods and old/new file versions of the 222-page Capital One PDF, survived tab changes and refresh, and restored a loaded PDF image. Search 2020-09 finds one of 52 sections. No real transactions imported or financial writes. Focused 27 checks, typecheck and scoped lint passed. The 99-page Andrews file also passed: search its 45 sections, inspect missing continuation page 13, switch to the separate receipt on page 42, refresh and restore the receipt/source, then reopen the statement. Unassigned rows were not included and no financial writes occurred. Private large-andrews-check.json records this. Commit the navigation/search work locally, then continue shared-case session/access recovery and incoming UI feedback. Do not stop at another commit.

## Active continuation: user instruction overrides the ready checkpoint

15 September 2026, after the user explicitly said not to stop. The five-minute heartbeat is ACTIVE through tonight. Do not pause it at a local commit or readiness label. The previous local checks and commits remain valid evidence, but do not substitute the checklist for continuing the actual user workflows. Read-only UI work now uses exact case edit/upload permissions. Real ordinary-member browser checks passed for read-only, edit-only and upload-only access with loaded original PDF images, source inspection and saved report download. No financial mutations; temporary member removed and revoked access denied. Focused access/component checks passed. Scoped lint and production build passed, with the existing chunk-size warning. Commit the access change locally, then continue large-statement review and incoming UI feedback. Local API restarted at PID 47213, session 73322; log /tmp/loupe-access-api.log. Private access-ui-check.json records the three exact permission combinations and zero financial writes. No new push instruction. Run focused checks for changes, not the completed broad suites.

## Ready checkpoint: final local financial release, 15 September 2026

The implemented financial workflow has reached its final local verification checkpoint. Read docs/releases/2026-09-15-financial.md and docs/loupe-financial-completion-plan.md for the exact scope and checked items. Do not repeat completed imports, reports or broad test runs. The user will test the UI after travelling home. No push without a new explicit request; the live server has not received these local changes. The schedule was briefly paused here and then restored after the user explicitly instructed continued work. Keep it ACTIVE until tonight’s deadline; do not pause at a ready label.
...
Private receipts: andrews-v12-coverage.json, andrews-v12-proposals-*.json, andrews-v12-differences-*.json, andrews-spacing-visual-check.json and andrews-spacing-browser.json. Previous v9/v11 snapshots remain intact. The existing 99-page source-only browser case c3120f3d-f029-4c44-928b-60d9ab4cb091 was reused without another upload. Local backend was restarted on 58002 with v12; frontend remains on 55174. Guide: 19,182 words, 21 synthetic images, closed by default.

Remaining in the larger PDF: four membership/signature pages, a July continuation with a missing predecessor in the supplied sequence, a summary-only page and an embedded teller deposit receipt. The smaller PDF includes membership/legal pages, a summary page and an OCR-damaged savings share identifier. Do not replace ODDO with 0000 automatically. The teller receipt on original page 42 has separate effective/printed dates, a deposit amount, previous/new/available balances and a masked account reference. It belongs in a supporting-document review, not automatically among statement payments or attached to an unmasked account. Only the Wells Fargo single-wire document currently has the separate saved-document review workflow.

Continue from these concrete source gaps and the existing expert-packet/independent-measurement acceptance requirements. Corpus counts and visual spot checks are not independently reviewed ground truth. External AI acceptance remains unverified. No public push, private documents or external provider calls.

## Previous continuation: Andrews payment shares and closure notices, 15 September 2026

The Andrews reader now recognises exact VISA PAYMENT headings as separate share sections, records their printed label/reference as account type Other and does not identify them as a credit-card account. Matching dated Closed notices are retained as source-linked account information. A closure must match the current share, account label and statement period. Its row is not a payment and cannot be imported as one. Metadata and citations include page, table and row identity; original review digests and exact cells are checked when reopening the source.

A section with no proposed payments and a printed closure can be saved without inventing an absent closing balance. Its opening balance, statement period and closure source are retained, with zero ledger rows. Excluding all payments from a populated proposal does not qualify. The review shows Save account closure and a PDF source button. The import receipt reports the closure date, including idempotent retry from retained original metadata. Statements > Review accounts > Review statements > Open statement and balances reopens the dated notice and highlighted original text. No schema migration is needed. Reader version is statement-review-v11.
...
Synthetic browser case e5bc909e-08af-4576-95d6-07425b5a961e, source file 001277e3-43fc-49f9-acb0-8d19138104c9, recorded source d848950c-028f-4a93-b702-f7f958ff490f. Local receipts: andrews-closure-browser.json, andrews-closure-persistence.json, andrews-real-closure-browser.json, andrews-v11-coverage.json, andrews-v11-proposals-*.json and andrews-v11-differences-*.json. Previous v9 snapshots are preserved. The guide now has 19,170 words and 21 synthetic images and stays closed unless opened. No private source or receipt is committed, no external provider call and no push. Local commits before this checkpoint: 5e3449ea, d0654289, 070e97a8 and 08b1028c.

Remaining source work: the Andrews collections still have unreadable/conflicting share or account headers, repeated copies and an embedded teller receipt. Keep unresolved ownership/dates/amounts explicit instead of moving those rows into neighbouring accounts. Wider institution/layout support and independent reference labels are still required before claiming comprehensive extraction accuracy. The existing original expert-packet acceptance, corpus release gate and optional external AI acceptance are separate outstanding items.

## Previous continuation: wire document review and blank-column import fix, 15 September 2026

Completed the supported Wells Fargo single wire-detail report workflow. Opening the PDF from Statement files displays its original page beside labelled editable details. Damaged or absent labels, redacted fields, currency and timestamps remain flagged. Currency is not inferred from a USD equivalent alone. Review corrections retain original text and PDF positions with their reasons. The investigator can save observations in Findings and optionally link an existing admitted payment with the same amount/currency and a recorded date within three days, with an explicit reason. A saved review never creates a ledger transaction.

The saved Workspace note holds one link to the wire PDF and, when selected, a second link to the existing payment's own statement. Source bytes, prepared reading revision, case permissions and the selected payment revision are checked before save. A deterministic request ID prevents duplicate notes on retries. The browser remembers a saved draft and offers an explicit new review for later corrections. Statement files show saved wire-review counts and a Findings link. Findings reopen original/saved fields and source images; readable reports include those comparisons and corrections. Report packages verify the wire's retained original digest and include the exact supporting PDFs. The guide has the procedure and a synthetic screenshot and remains closed unless opened.

During the browser journey, a new borderless synthetic statement exposed positional column collapse: a balance in the third present cell was treated as a credit. Fixed in local commit 070e97a8. Text-alignment tables now use the printed header bands; missing/overlapping positions are flagged without index fallback. Original source indices remain unchanged for field corrections. Explicit account/period/currency headings are excluded only after a transaction header is recognised; unknown pre-table text remains reviewable. Reader VERSION is statement-review-v10. Issuer-specific Andrews/Merrick/Capital One readers are separate.

Validation: 60 unique focused backend checks across wire proposal/save, statement proposal/persistence, file status and route permissions; 18 focused frontend checks for the new form, saved state, file list, reports and source packages; TypeScript; scoped lint; guide generation (18,973 words, 20 synthetic images); production build (11.29 seconds, existing large-chunk warnings). No full suite. The synthetic browser case dd47f510-c4f6-4083-863a-deabcac1a76f contains exactly one USD 120 payment (opening 100, closing 220) and one saved wire note 230d5959-fcfb-54d3-a21b-cd21295ae65c. Its bundle includes both original PDFs byte for byte. The supported real wire PDF was freshly uploaded into an isolated source-only local case and inspected with no financial import or saved real finding. Its amount/date were read and damaged currency, timestamp and two party headings were flagged. Private receipts: wire-browser.json, wire-persistence-receipt.json, real-wire-browser.json, real-wire-current-proposal.json and wire-finding-with-pdfs.zip under data/local-runtime. PDFs and private receipts are not committed.

The first synthetic import browser run exposed the actual column bug above. A later browser helper needed explicit accessible labels for populated textarea fields, which were added; the export helper was corrected to open Statement files from Statements. These reruns reused existing synthetic files/payment. Local backend is running through scripts/local_app.py backend on 58002; frontend on 55174. No external provider calls or push.

Continue with unsupported Andrews account sections and other source document layouts. The 99-page Andrews collection still has unreadable/conflicting sections and VISA PAYMENT account headings; the 24-page collection has a damaged share heading and repeated copies. Do not treat remaining detection counts as independent accuracy measurements. Independent reviewed labels, measured extraction release acceptance and applicable original full expert-packet acceptance remain open; do not mark the whole financial programme complete from these checks.

## Previous continuation: Andrews continuation pages and printed page order, 15 September 2026

A second pass through the newly supplied PDFs found readable continuation pages whose text layer omitted the Andrews logo. Such a page may now continue an established account section only with matching account/period, a preceding continuation notice, the expected page relationship and no conflicting printed number. A page without the logo cannot independently establish an Andrews statement. Exact Account ·Statement and .Andrews punctuation variants are recognised. Whitespace within a printed period and around monetary signs/decimal points is accepted without changing any digit. Damaged letters, signs, separators and ambiguous monetary boundaries remain exceptions. Original source cells are unchanged. Reader version is statement-review-v9.

The larger PDF stores the first five December pages as printed pages 4, 3, 2, 1, 5. The catalog now orders a physically contiguous block only when every page has a matching account/period and its printed page numbers form a complete unique sequence starting at 1. Unreadable headings end the block; gaps/repeated numbers do not get guessed. Proposals and imported transaction row indexes follow that reading order, while all original PDF page addresses and field rectangles remain unchanged. The viewer starts at the first statement page, keeps original-file Previous/Next navigation, and explains the different printed order. The supplied December section now includes 142 payments from original PDF pages 38, 37, 36, 35, 39 and 40. The following summary-only page remains separately visible.

Whole-page browsing now says Original PDF page N. The earlier text incorrectly implied that a selected transaction was missing its position even when the investigator had only opened a page. A genuinely unlocated transaction still shows its location limitation. The user guide explains spacing, continuations and printed/original page order. Documentation remains closed unless opened by the user.

Validation: 21 unique focused backend checks across the affected reader/catalog/import tests and 19 source-view frontend checks passed, plus TypeScript, scoped ESLint, guide generation and production build. A synthetic database import confirms transaction order and balances are saved with the original PDF pages [2, 1]. No full suite. The first version of that synthetic test needed its extraction-job ID and outer page rectangle corrected to satisfy existing source guards; the production guards were unchanged.

A fresh upload of the 99-page source was checked in isolated local case c3120f3d-f029-4c44-928b-60d9ab4cb091, file 02b699bf-9d17-4e3f-8257-39ad243317ec. Browser checks passed for 37 account/period choices, 95 October payments, all 17 payments on PDF page 29, the 49.25 USD refund and 674.74 USD final printed balance, loaded source image/highlight, page navigation, December's 142 payments and the corrected whole-page caption. No real transactions were imported. An initial browser helper timed out because its filename expression contained two spaces while the accessible name collapsed them to one; the existing successful upload was reused after fixing that expression. Final console errors: none. Desktop screenshots were visually checked.

Retained-source comparison now identifies 37 sections / 1,950 proposed rows in the 99-page file, with 1,897 amounts, 1,891 balances, 216 rows requiring review and 19 wholly/partly unclassified sources. The 24-page file remains 15 sections / 381 proposed rows, with 373 amounts, 375 balances, 34 rows requiring review and 6 unclassified sources. All previously populated dates, amounts, directions and balances are unchanged. The larger file gained 729 proposed rows compared with the first Andrews checkpoint. Visual spot-checks match all 9 payments on page 19 and 17 on page 29, including their final balances. These are local detection/regression/spot-check results, not independently reviewed accuracy measurements.

Local receipts: andrews-v9-coverage.json, andrews-v9-proposals-*.json, andrews-v9-differences-*.json, andrews-v9-visual-sample.json and andrews-large-browser.json under data/local-runtime. Reusable scripts: loupe-andrews-coverage.py and loupe-andrews-large-browser.cjs. Original PDFs and private extracts remain excluded from commits. Local backend/frontend are running on 58002/55174 with these changes. No external provider calls or public push.

Remaining: unsupported VISA PAYMENT sections and damaged/missing headings in the Andrews sources; a separate workflow for the buyer-wire report and the teller receipt found among the statements; independent reviewed labels and measured release acceptance; applicable original expert-packet acceptance and optional live-provider acceptance. Do not mark these complete or treat extra detected rows as a completeness guarantee. The first Andrews checkpoint is local commit 5e3449ea; this follow-up is a separate local commit.

## Previous continuation: Andrews savings/checking support, 15 September 2026

The newly supplied Andrews PDFs exposed a second required statement format. The reader now recognises the measured Andrews account/period heading and exact BASE SHARE SAVINGS / FREE CHECKING sections. Each share has its own account reference, statement choice, selected source rows, balances and import. A repeated opening heading starts a separate statement occurrence even when the account and dates match, preventing two distinct copies from being concatenated. Continuation pages require an adjacent original PDF page, matching account and period, the preceding continuation notice, and no conflicting printed page number. Unreadable/unknown share headings reset ownership rather than inheriting the previous account. Other account sections, account applications and unassigned pages remain outside the selected import and available in the PDF.

Proposals retain wrapped descriptions and their original continuation cells, including across a valid page boundary. Exact readable amounts and running balances are separated even when the native OCR joined their cells. Damaged digits/signs/separators remain exceptions; a readable amount or balance is retained independently when the other fails. Refund signs, missing withdrawal signs, out-of-period/damaged dates, second unlabelled dates and running-balance differences are checked. No missing transaction year, source digit or second date role is silently repaired. Version is statement-review-v8.

Import duplicate checks compare the selected physical account sections, so importing savings does not block checking on the same page. Physical positions remain comparable when rereading renumbers rows or tables. If the coordinate systems cannot be compared, a shared page requires review; row numbers from different readings are never treated as proof that sections do not overlap. Overlapping readings and older whole-table imports still require the existing replacement flow. Account type is retained as savings/checking instead of assuming every recognised period is a credit card. Opening/closing controls retain their PDF sources using the asset-balance convention. A section with no proposed payments can save account coverage and balances only when both balance controls are present and the reviewed values match. Excluding an ordinary transaction list cannot bypass that restriction.

The original view renders these unheaded lines at their measured horizontal positions. It adds no invented column headers and does not split a compound source cell into fabricated highlights. Dates/amounts remain on one line; the desktop check shows dates, descriptions and balances together with the file sidebar open. Unlocated source text remains accessible. The existing correction controls stay separate. Guide text now explains choosing each share, repeated copies, continuation pages, corrections and saving a statement without payments. Documentation panels remain closed.

Validation: 75 unique affected backend checks passed across Andrews/catalog/card/Merrick/import/proposal, plus 23 unique affected frontend checks, TypeScript, scoped ESLint, guide generation and production build. Subsequent narrow checks covered independent damaged amount/balance retention and unlocated display. No full suite. A fresh upload of the 24-page private Andrews file into isolated local case fc108cd0-a104-4404-b2e3-6c23ad77c762, file ad047151-e958-45e9-8aad-20628c925964, showed 15 choices, 7 first-savings payments and correct separate checking scope. Clicking the compound source value highlighted the actual PDF location; next/previous page navigation passed with no console errors. No real transactions were imported or corrected. Synthetic writer checks cover two shares imported from one page, repeat confirmation, persisted balance sources, rereads with changed row numbers and zero-payment statements. A complete synthetic browser import of a no-payment statement saved both 100.00 USD balances and opened Statements > Review accounts automatically. Case 54f25b3d-5af8-4db3-a461-dd28fc8a1c99, file bf53f5c2-dd80-4613-8641-22ab01c0e032, receipt andrews-zero-payments-browser.json. The resulting account view and the source-layout screenshots were visually checked.

Read-only native-text comparison: the 99-page Andrews file now identifies 33 account/period sections and proposes 1,221 rows; 266 rows have review issues and 55 source tables are wholly or partly unclassified. The 24-page file identifies 15 sections and proposes 381 rows; 65 rows have review issues and 6 source tables are wholly or partly unclassified. These are detection counts, not accuracy or completeness measurements. Hashes confirm both original PDFs unchanged. Some unclassified pages contain forms or other account types; others contain unreadable or conflicting headings. The larger file is not automatically complete.

A bounded local 300-dpi Tesseract comparison on pages 5 and 12 of the smaller file recovered the damaged savings-share heading on page 12 but introduced different numeric OCR errors. Do not recommend blanket replacement or claim OCR rereading always improves accuracy. Both sample readings remain private and were not written into the case. The supplied buyer-wire PDF was visually inspected: it is a Wells Fargo wire-detail report with a credit amount, value date, sending/receiving parties and redacted references, not a bank statement. A dedicated documentary-payment interpretation remains to be designed and implemented.

Local receipts/scripts: data/local-runtime/andrews-support-comparison.json, andrews-proposals-*.json, andrews-new-statements-browser.json, andrews-first-savings-proposal.json, andrews-page-image-samples.json, loupe-andrews-browser.cjs and loupe-andrews-ocr-sample.py. Private PDFs, extracted content, screenshots and receipts are excluded from commits. Remaining: broader formats/unknown shares and OCR coverage, separate wire-document support, independently reviewed corpus measurements and release acceptance, applicable original expert-packet acceptance and optional external-provider acceptance. No external provider request or public push.

## Previous continuation: readable tracing packages and newly supplied statements, 14 September 2026

Review packages now include a readable report for each selected saved tracing calculation. Each report shows recorded opening funds and reasons, claim assignments, payment order, transfer choices and timing assumptions, separate method results, per-account figures, asset purchases and resales. Payment links lead to captured values, original date fields and source references in the same report. The original scenario JSON stays byte-for-byte unchanged. New reports have manifest hashes and a versioned optional flag; older packages rebuild without adding new files. Unknown report versions are refused and altered derived reports are identified. Asset allocation displays in both package and standalone reports now separate other recorded funds from unidentified/unfunded portions, avoiding double-counting the saved outside-claims total.

The user supplied more local PDFs. The 222-page Capital One collection contains the exact account/page from the earlier screenshot. A read-only native-text inspection found that later Secured Card and Platinum Secured Card headings were unsupported. Both catalog and layout recognition now share an exact heading matcher. Damaged account digits, unknown product names, conflicting accounts and invalid periods remain unclassified. Reader version is statement-review-v7. Recognition on this supplied file increased from 7 to 52 periods, with 301 proposed rows; this is detection coverage, not independently measured accuracy. The exact screenshot period has the three correct dates and amounts (one credit, two debits), with advertising outside the transaction tables.

Validation: 57 affected package/report/router backend checks, 21 affected statement checks, 13 frontend checks, TypeScript, scoped lint, guide generation and production build passed. No full suite. Browser assembly with a saved synthetic scenario, ledger/history and explicitly synthetic validation downloaded successfully and independently rebuilt. Desktop/mobile report checks confirmed source links, purchase/resale content and no overflow or console errors. The earlier browser helper needed its current parent Reports disclosure opened; the Capital One helper needed to wait for the currency selector. Both completed after script corrections; these were not application defects.

The exact supplied Capital One PDF was uploaded into a new isolated local review case, 62981b03-9fdf-4fad-9d1b-6c509d6cbff2, file 1a269d6d-0598-431a-a188-334b46ae6eb1. It prepared all 222 pages, showed 52 period choices and opened the screenshot page correctly. No real transactions were imported or corrected. Private local receipts: capital-exact-8160-browser.json, capital-exact-8160-proposal.json, capital-secured-heading-comparison.json, readable-scenario-package-verification.json, readable-scenario-report-package.zip, new-statement-inventory.json and new-source-*.json. Original inputs and PDFs stay outside commits.

Remaining concrete development: the new Andrews collections include multiple savings/checking shares, continuation lines and compound amount/balance cells, plus account-opening forms. They are not yet supported by the automatic statement profile. The buyer-wire document also needs a separate financial-document interpretation, not a bank-statement assumption. Investigate these next. The missing exact Capital One source is now resolved. Independent reviewed labels, measured release acceptance, optional live-provider acceptance and applicable whole expert-packet acceptance remain open. No external provider requests or push were made.

## Previous continuation: Capital One fees and separate date corrections, 14 September 2026
