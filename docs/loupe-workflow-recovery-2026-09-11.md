...
## Saved indirect workpapers, 11 September follow-up

Findings now opens existing indirect workpapers with their stored inputs, result, checks and original source files. Users can create a separate revised copy, calculate again and save a new note linked to the original. New notes include the method definition; older notes use a checked compatibility path. The viewer also downloads a readable HTML report and the original JSON calculation file.

A local browser journey reopened an older saved workpaper, inspected its source, downloaded matching JSON, retained a copied edit across financial tabs, saved and reopened a 50.00 GBP revised result and confirmed that the original 40.00 GBP note was unchanged. No ledger edits occurred. The HTML report rendered its saved amounts, checks and references. Fifteen focused tests and scoped lint passed; the completed checklist and limitations are in `docs/loupe-financial-redesign-plan.md`. No full-suite rerun was performed for this follow-up.

The final review also found an HTTP browser incompatibility affecting financial digest checks and two request-ID controls. These now use shared native-or-JavaScript SHA-256 and secure random-byte UUID helpers. The pinned `@noble/hashes` dependency supplies the fallback; no integrity checks are skipped. All 79 affected checks passed, the production build passed, and an intercepted HTTP-origin browser opened the saved workpaper and downloaded a PDF report package with matching hashes. Type/lint checks and the guide render passed. No deployment completion is claimed from these local checks.
