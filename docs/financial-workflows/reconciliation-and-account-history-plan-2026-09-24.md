...
Commit `62f7db3a40104b95e67da49a3cbc644e385d2e7f` was pushed to `integration/evidence-main-reunion`, triggering automatic deployment. The user confirmed the live address and signed in for browser verification. The live Updates screen identifies that checkout as deployed/latest, but its captured deployment log reports a frontend TypeScript failure in `ReconciliationJourney.browser.test.tsx` and a restart serving the previous build. The checkout label alone therefore does not establish the running frontend revision. A forced local typecheck reproduced the same callback-assignment narrowing error at the native-PDF popup cleanup. Replacing the callback-captured nullable variable with a typed reference preserves the test behaviour; `tsc -b --force`, the production build and scoped lint are checked before the corrective push. Authenticated workflow acceptance continues after the corrected frontend is observed. Acceptance edits remain restricted to the previously authorized Neil finance test case; client cases remain read-only. The earlier production cleanup audit remains separate and unverified. No manual deployment or ingestion cancellation was attempted; the observed deployment log passed its idle-ingestion gates.

## Authenticated verification and follow-up — 25 September 2026

- Corrective commit `ded066c9` deployed successfully. The authenticated Updates log records a completed production frontend build, idle ingestion gates, healthy services and successful deployment in 174 seconds. The running build is grumpy-alpaca (`ded066c`). No manual deployment was initiated.
- At 1440 × 900 in the authorized test case, saved balance/page edits persist and account history separates currencies, saved balance observations and missing months. Existing historical bank-name/account-number differences remain separate; this verification does not merge them.
- A saved no-payment period still requires review after explicit confirmation. The existing receipt did not explain why. The follow-up returns the stored admission blockers beside the saved details, with save/reopen regression coverage. Confirmation is not assumed to have passed simply because values were saved. Its actual live blocker remains to be inspected after this follow-up deploys.
- Opening the current PDF page created a separate protected original tab while preserving an unfinished row. The browser automation policy prevents inspecting the resulting blob URL, so native PDF rendering is not certified by this live check. Selectable extracted page text and all source pages were checked in the working review.
- Entering an amount and using the problem filter keep the manual draft visible; excluded rows remain off. An explicit Discard added row action now removes only a manual draft row and preserves other corrections. No test payment was imported.
- Removed obsolete text saying reading issues cannot block import. New-payment import now consistently explains the reconciliation requirement.
- Follow-up validation: 129 backend tests plus 11 subtests; 58 frontend tests, forced TypeScript build, production build and scoped lint pass. The manual-row regression includes discard and authenticated draft reopening. Production data cleanup and the original live Apply USD incident's root cause remain unverified.
