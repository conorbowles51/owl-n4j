...
const output=path.join(root,'data/local-runtime/statement-draft-journey-check.json');
const resume=process.argv[2]==='--resume-interest';
(async()=>{if(fs.existsSync(output)&&!resume)throw Error('Review record exists; inspect before retrying any write');const intake=JSON.parse(fs.readFileSync(path.join(root,'data/local-runtime/draft-controls-pdf-intake-check.json')));if(intake.case_id!=='2da970d0-b96b-4928-a23d-14f5e03407bb')throw Error('Unexpected acceptance case');
 ...
 const draftSaved=page.waitForResponse(r=>r.url().includes('/statement-draft?')&&r.request().method()==='PUT');
 await page.getByRole('button',{name:'Save statement controls to case',exact:true}).click();
 const draftResponse=await draftSaved;const savedDraft=await draftResponse.json();if(!draftResponse.ok()||savedDraft.statement_scopes.length!==1)throw Error('Statement draft save failed');
 report.draft_revision=savedDraft.revision;report.draft_saved=true;save();
 await page.goto(`http://127.0.0.1:55174/cases/${intake.case_id}/financial`);
 await page.getByRole('button',{name:'Open PDF readings',exact:true}).click();
 await page.getByRole('listitem').filter({hasText:'1 possible transactions'}).getByRole('button',{name:'Open readings',exact:true}).click();
 await page.getByRole('button',{name:'Preview finalization',exact:true}).click();
 await page.getByRole('button',{name:'Load saved statement controls',exact:true}).click();
 report.draft_reloaded=true;save();
 ...
 const download=page.waitForEvent('download');await page.getByRole('button',{name:'Download ledger snapshot',exact:true}).click();await(await download).saveAs(path.join(root,'data/local-runtime/statement-draft-journey-export.zip'));report.export_downloaded=true;save();
