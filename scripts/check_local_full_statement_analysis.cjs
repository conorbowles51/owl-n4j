...
(async()=>{const sample=JSON.parse(fs.readFileSync(path.join(root,'data/local-runtime/full-statement-review-check.json')));
 ...
 const downloaded=page.waitForEvent('download');await page.getByRole('button',{name:'Download ledger snapshot',exact:true}).click();await(await downloaded).saveAs(path.join(root,'data/local-runtime/full-statement-final-export.zip'));
