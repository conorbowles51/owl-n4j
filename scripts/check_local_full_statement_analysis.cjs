...
(async()=>{const sample=JSON.parse(fs.readFileSync(path.join(root,'data/local-runtime/full-statement-review-check.json')));
 ...
 await page.getByLabel('Find in loaded rows',{exact:true}).fill('TSP*COPPER');await page.getByText(/1 of 3 loaded rows match/).waitFor();
 await page.getByRole('button',{name:'Reset table view',exact:true}).click();
 await page.getByLabel('Table order',{exact:true}).selectOption('amount-desc');
 if(!(await page.getByTestId('ledger-table').locator('tbody tr').first().innerText()).includes('PAYMENT'))throw Error('Amount ordering disagrees with exact source values');
 await page.getByLabel('Table direction',{exact:true}).selectOption('debit');await page.getByText(/2 of 3 loaded rows match/).waitFor();
 await page.getByRole('button',{name:'Reset table view',exact:true}).click();
 ...
 await page.getByRole('region',{name:'Money by ordering date',exact:true}).screenshot({path:'/tmp/loupe-real-date-flow-chart.png'});
 ...
 await page.getByRole('region',{name:'Money by source label',exact:true}).screenshot({path:'/tmp/loupe-real-label-flow-chart.png'});
 await page.getByLabel('Include chart group Counterparty not recorded',{exact:true}).uncheck();
 await page.getByText(/0 of 1 groups selected/).waitFor();
 await page.getByLabel('Include chart group Counterparty not recorded',{exact:true}).check();
 await page.getByRole('button',{name:'Inspect chart group Counterparty not recorded',exact:true}).click();
 await page.getByRole('button',{name:'Open chart posting 1',exact:true}).click();
 const sourceDialog=page.getByRole('dialog',{name:'Ledger source',exact:true});await sourceDialog.waitFor();await sourceDialog.getByRole('button',{name:'Close',exact:true}).click();
