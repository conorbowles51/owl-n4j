...
(async()=>{const browser=await chromium.launch({headless:true});try{
 ...
 await page.screenshot({path:'/tmp/loupe-auto-column-scan.png'});
 fs.writeFileSync(path.join(root,'data/local-runtime/automatic-page-scan-results.json'),JSON.stringify(scans,null,2));
 if(writes)throw Error('Scan wrote financial data');
 const report={case_id:caseId,automatic_columns:true,requested_pages:108,checked_pages:all.filter(p=>p.checked).length,unchecked_pages:all.filter(p=>!p.checked).length,suggested_rows:scans.reduce((n,s)=>n+s.suggested_rows,0),first_statement_dated_suggestions:2,undated_interest_not_nominated:true,financial_writes:writes};
 fs.writeFileSync(path.join(root,'data/local-runtime/automatic-page-scan-check.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report));
