...
(async () => {
  ...
  try {
    ...
    for (let attempt=0;attempt<2;attempt++) {
      await page.getByRole('button',{name:'Suggest column meanings',exact:true}).click();
      if(await page.getByLabel('Column 1 meaning',{exact:true}).inputValue()!=='unknown') throw new Error('A suggested meaning was automatically applied');
      await page.getByRole('button',{name:'Use Transaction date for column 1',exact:true}).click();
      await page.getByRole('button',{name:'Use Amount for column 2',exact:true}).click();
      for(const number of [1,2,3]) if(await page.getByLabel(`Select source row ${number}`,{exact:true}).isChecked())throw new Error('Suggestion selected a source row');
      await page.getByLabel('Select source row 2',{exact:true}).check();
      if(attempt===0) await page.screenshot({path:'/tmp/loupe-neilbyrne-header-suggestions-ui.png'});
      ...
      if(data.original.proposal.rows[0].row_index!==1 || data.original.proposal.columns[0].meaning!=='transaction_date' || data.candidates[0].original.cells[1].text!=='1234') throw new Error('Original source row mismatch');
    ...
    await page.getByRole('button',{name:'Review source row 2',exact:true}).click();
    ...
    fs.writeFileSync(path.join(root,'data/local-runtime/header-ui-check.json'),JSON.stringify(summary,null,2)+'\n');
