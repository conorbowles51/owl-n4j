...
expected=json.loads((root/'second-real-pdf-review-check.json').read_text())
with ZipFile(root/'second-real-pdf-sample-export.zip') as z:
...
assert sorted(r['reading']['amount_minor'] for r in history['reviews'])==['18000','6162']
assert all(r['actor'] and r['reason'] and r['previous_revision'] for r in history['reviews'])
assert any('- $180.00' in str(c['snapshot']) for c in history['candidates'])
assert {(r['reading']['amount_minor'], r['reading']['direction'], r['reading']['transaction_date']) for r in history['reviews']} == {('18000', 'credit', '2020-05-30'), ('6162', 'debit', '2020-05-29')}
assert hashlib.sha256((root.parent/'loupe-test-pdfs/statements - Copy (3)_Redacted.pdf').read_bytes()).hexdigest() == 'f9292c6e6c360b0bb6dc1534019a682bc0e0f04f656a36f723fff60f64a27c7b'
...
(root/'second-review-history-export-verification.json').write_text(json.dumps(report,indent=2));print(json.dumps(report))
