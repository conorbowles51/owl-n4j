...
(async()=>{const previous=fs.existsSync(reportPath)?JSON.parse(fs.readFileSync(reportPath)):null;if(previous&&previous.preparation)throw Error("Preparation already submitted; inspect saved job without resubmission");const browser=await chromium.launch({headless:true});try{
 ...
 if(!caseId){const created=await page.request.post('http://127.0.0.1:58002/api/cases',{headers,data:{title:`LOCAL ACCEPTANCE — ${sample} supplied PDF review`,description:'Isolated offline workflow test with a copy of supplied evidence.'}});if(!created.ok())throw Error('Case creation failed');caseId=(await created.json()).id;}
 const filename=sample==='first'?'006406-006461 Hopper Lashika 0225 esubp resp_Redacted.pdf':'statements - Copy (3)_Redacted.pdf';const original=path.join(root,'data/loupe-test-pdfs',filename);const digest=()=>crypto.createHash('sha256').update(fs.readFileSync(original)).digest('hex');const originalHash=digest();
