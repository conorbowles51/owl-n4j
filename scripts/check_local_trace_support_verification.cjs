...
(async()=>{
 ...
 try{
  ...
  await page.getByText('Check a saved review package',{exact:true}).click();
  await page.getByLabel('Review package or tracing audit ZIP',{exact:true}).setInputFiles(archive);
