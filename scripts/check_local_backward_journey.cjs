...
(async () => {
  ...
  try {
    ...
    const sourceRead=page.waitForResponse(r=>r.url().includes('/source?'));await results.getByRole('button',{name:'Open hop 1 credit',exact:true}).first().click();const sourceResponse=await sourceRead,sourceData=await sourceResponse.json();if(!sourceResponse.ok()||sourceData.transaction_id!==fixture.pairs[0].credit_id)throw Error('Backward source scope differs');await page.keyboard.press('Escape');
    ...
    const output=path.join(root,'data/local-runtime/backward-trace-scenario.json');await(await download).saveAs(output);
    console.log('Downloaded');await page.screenshot({path:'/tmp/loupe-backward-trace-results.png'});
    ...
    const verified={case_id:fixture.case_id,synthetic:true,backward_timing_used:true,default_refused:true,source_navigation:true,accounts:3,postings:7,selected_hops:2,remaining_minor:expected,download_verified:true,stale_results_cleared:true,financial_writes:writes};
    fs.writeFileSync(path.join(root,'data/local-runtime/backward-trace-ui-check.json'),JSON.stringify(verified,null,2));console.log(JSON.stringify(verified));
