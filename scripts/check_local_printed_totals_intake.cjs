...
const sample='printed-totals';const reportPath=path.join(root,'data/local-runtime/printed-totals-intake-check.json');
(async()=>{const previous=fs.existsSync(reportPath)?JSON.parse(fs.readFileSync(reportPath)):null;if(previous&&previous.preparation)throw Error("Preparation already submitted; inspect saved job without resubmission");const browser=await chromium.launch({headless:true});try{
 ...
 if(!caseId){const created=await page.request.post('http://127.0.0.1:58002/api/cases',{headers,data:{title:`LOCAL ACCEPTANCE — ${sample} synthetic printed total review`,description:'Isolated offline workflow test with a generated synthetic statement; not bank evidence.'}});if(!created.ok())throw Error('Case creation failed');caseId=(await created.json()).id;}
 const filename='synthetic-printed-totals.pdf';const original=path.join(root,'data/local-runtime',filename);const digest=()=>crypto.createHash('sha256').update(fs.readFileSync(original)).digest('hex');const originalHash=digest();
 ...
 if(digest()!==originalHash)throw Error('Original PDF changed');report.original_unchanged=true;report.ui_reaches_source_selection=true;fs.writeFileSync(reportPath,JSON.stringify(report,null,2));await page.screenshot({path:`/tmp/loupe-neilbyrne-${sample}-real-pdf-intake.png`});console.log(JSON.stringify(report));
