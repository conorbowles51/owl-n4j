...
def _require_case_access(case_id: str, user: User, db: Session):
    ...
    try:
        get_case_if_allowed(db, _case_uuid(case_id), user)
    except CaseNotFound:
...
def _resolve_attachments(db: Session, case_id: str, items: List[dict]) -> None:
    """
    ...
    records = EvidenceDBStorage.get_by_cellebrite_file_ids(
        db,
        _case_uuid(case_id),
        list(all_ids),
    )
...
def get_comms_thread_detail(
    ...
    _resolve_attachments(db, case_id, result.get("items", []))
    return result
...
def resolve_comms_attachment(
    ...
    _require_case_access(case_id, current_user, db)
    records = EvidenceDBStorage.get_by_cellebrite_file_ids(db, _case_uuid(case_id), [file_id])
...
def get_event_detail(
    ...
    if file_ids:
        rec_map = EvidenceDBStorage.get_by_cellebrite_file_ids(
            db,
            _case_uuid(case_id),
            list(file_ids),
        )
...
def _cellebrite_files_for_case(db: Session, case_id: str, report_keys: Optional[List[str]]) -> List[dict]:
    """Return all cellebrite evidence records in the case (optionally scoped to reports)."""
    return EvidenceDBStorage.list_cellebrite_files(
        db,
        _case_uuid(case_id),
...
def get_cellebrite_files(
    ...
    if capture_after or capture_before:
        def _ts_prefix(rec):
            ts = rec.get("capture_time") or rec.get("creation_time")
