...
from config import TRIAGE_ALLOWED_ROOTS
...
def _is_allowed_browse_path(path: Path) -> bool:
    if not TRIAGE_ALLOWED_ROOTS:
        return True
    for root in TRIAGE_ALLOWED_ROOTS:
        try:
...
def _require_case_owner(case_id: str, current_user: User, db: Session) -> dict:
    case = triage_service.get_case(case_id, db=db)
    if not case:
...
def _require_target_case_upload_access(target_case_id: str, current_user: User, db: Session) -> None:
    try:
        check_case_access(
            db,
            UUID(target_case_id),
        ...
        raise HTTPException(status_code=400, detail="Invalid target_case_id")
...
async def browse_directory(
    ...
    if not _is_allowed_browse_path(target):
        raise HTTPException(status_code=403, detail="Path is outside allowed triage roots")
...
async def list_triage_cases(
    ...
    cases = triage_service.list_cases(owner=current_user.email, db=db)
    return {"cases": cases}
...
async def delete_triage_case(
    ...
    if case.get("created_by") != current_user.email:
        raise HTTPException(status_code=403, detail="Access denied")
    triage_service.delete_case(case_id, db=db)
...
async def start_scan(
    ...
    if case.get("created_by") != current_user.email:
        raise HTTPException(status_code=403, detail="Access denied")
    try:
        task_id = triage_service.start_scan(case_id, resume=request.resume, db=db)
...
async def get_scan_stats(
    ...
    if case.get("created_by") != current_user.email:
        raise HTTPException(status_code=403, detail="Access denied")
    try:
        stats = triage_service.get_scan_stats(case_id, db=db)
        return stats
...
async def start_classification(
    ...
    if case.get("created_by") != current_user.email:
        raise HTTPException(status_code=403, detail="Access denied")
    try:
        task_id = triage_service.start_classification(case_id, db=db)
...
async def upload_hash_set(
    ...
    if case.get("created_by") != current_user.email:
        raise HTTPException(status_code=403, detail="Access denied")
    count = triage_service.upload_hash_set(
        request.name,
        request.hashes,
        created_by=current_user.email,
        db=db,
...
async def list_hash_sets(
    ...
    sets = triage_service.list_hash_sets(db=db)
...
async def generate_profile(
    ...
    if case.get("created_by") != current_user.email:
        raise HTTPException(status_code=403, detail="Access denied")
    try:
        task_id = triage_service.generate_profile(case_id, db=db)
...
async def get_profile(
    ...
    if case.get("created_by") != current_user.email:
        raise HTTPException(status_code=403, detail="Access denied")
    profile = triage_service.get_profile(case_id, db=db)
    if not profile:
...
async def list_templates(
    ...
    return {"templates": triage_service.list_templates(db=db)}
...
async def apply_template(
    ...
    if case.get("created_by") != current_user.email:
        raise HTTPException(status_code=403, detail="Access denied")
    try:
        stages = triage_service.apply_template(request.template_id, case_id, db=db)
...
async def delete_template(
    ...
    deleted = triage_service.delete_template(template_id, db=db)
    if not deleted:
...
async def ingest_preview(
    ...
    if case.get("created_by") != current_user.email:
        raise HTTPException(status_code=403, detail="Access denied")
    _require_target_case_upload_access(request.target_case_id, current_user, db)
