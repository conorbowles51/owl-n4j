...
def update_payment_labels(session, *, case_id, request, actor):
    ...
    try:
        ...
        for row in rows:
            ...
            before = {key: current.get(key, '') for key in row_changes}
            if before == row_changes:
                continue
...
def payment_label_view(row, account=None):
    ...
    other = row.counterparty_raw or ''
    sender, recipient = (other, own) if row.direction == 'credit' else (own, other)
    return dict(category=labels.get('category', ''),
                from_name=labels.get('from_name') or sender,
                to_name=labels.get('to_name') or recipient,
                label_version=labels.get('version', 0))
