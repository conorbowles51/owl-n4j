...
class Neo4jService:
    ...
    def get_cellebrite_comms_tally(
        self,
        case_id: str,
        report_keys: Optional[List[str]] = None,
        from_keys: Optional[List[str]] = None,
        ...
        """
        Per-contact communication tally + a "most contacted" ranking for the
        Comms Center, computed under the SAME filter set the feed uses so the
        numbers update live as the analyst filters.

        This is the sibling of get_cellebrite_comms_envelope: it reuses the
        envelope's exact param + filter-fragment block and its per-type MATCH
        shapes (strict call/email joins, OPTIONAL chat recipient) so the tally
        stays in lockstep with the envelope total and /comms/between — same
        filters, same DISTINCT semantics. The only difference is the grouping:
        instead of bucketing DISTINCT comms by day, we bucket them by
        counterparty + source_app + direction.

        Direction (matches the entities with_counts split — we do NOT resolve
        the device owner to compute it):
          • inbound  (from the contact) = the contact is the SENDER side
            (CALLED / SENT_MESSAGE / EMAILED)
          • outbound (to the contact)   = the contact is the RECIPIENT side
            (CALLED_TO / received in a chat via PARTICIPATED_IN / SENT_TO)

        Owner determination is used ONLY to tag each row's is_owner so the UI
        can drop the device owner from the ranking (an owner is never their own
        top contact) and so the case-level `totals` frame as the owner's
        inbound/outbound — never to compute direction.

        Returns:
          {
            contacts: [ {key, name, is_owner, call_in, call_out,
                         message_in, message_out, email_in, email_out,
                         total, by_platform: {app: n}} ],   # non-owner, ranked
            owners:   [ ...same shape... ],                  # owner rows, aside
            totals:   {message_in, message_out, call_in, call_out,
                       email_in, email_out, total, by_platform: {app: n}},
            contact_count: <n non-owner contacts>,
            truncated: <n non-owner contacts dropped past rank_limit>,
          }
        """
        active_types = set(types or ["message", "call", "email"])

        # --- Reuse the envelope's param + filter-fragment builder verbatim ---
        # so the tally is gated identically to /comms/between and the envelope.
        params: Dict[str, Any] = {"case_id": case_id}
        rk_filter_msg = rk_filter_call = rk_filter_email = ""
        if report_keys:
            rk_filter_msg = " AND msg.cellebrite_report_key IN $report_keys"
            rk_filter_call = " AND c.cellebrite_report_key IN $report_keys"
            rk_filter_email = " AND e.cellebrite_report_key IN $report_keys"
        ...
        # Per-contact accumulator, keyed by Person.key.
        acc: Dict[str, dict] = {}

        def _row(key: str, name: Optional[str], is_owner: bool) -> dict:
            r = acc.get(key)
            if r is None:
                r = {
                    "key": key,
                    ...
                    "call_in": 0, "call_out": 0,
                    "message_in": 0, "message_out": 0,
                    "email_in": 0, "email_out": 0,
                    "by_platform": {},
                }
                acc[key] = r
            else:
                # Prefer a real (non-key) name if a later leg carries one, and
                # let any owner-tagged leg promote the row to owner.
                if (not r["name"] or r["name"] == key) and name and name != key:
                    r["name"] = name
                if is_owner:
                    r["is_owner"] = True
            return r

        def _apply(rec, bucket: str) -> None:
            key = rec["key"]
            if not key:
                return
            n = int(rec["n"] or 0)
            if n <= 0:
                return
            r = _row(key, rec.get("name"), bool(rec.get("is_owner")))
            r[bucket] += n
            app = rec.get("app") or "Unknown"
            r["by_platform"][app] = r["by_platform"].get(app, 0) + n

        with self._driver.session() as session:
            if "message" in active_types:
                # Same MATCH shape as the envelope's message leg (msg anchored on
                # case_id → index seek; OPTIONAL chat recipient). Inbound credits
                # the sender; outbound credits each non-sender chat participant.
                msg_match = f"""
                    MATCH (sender:Person)-[:SENT_MESSAGE]->(msg:Communication)-[:PART_OF]->(chat:Communication)
                    OPTIONAL MATCH (recipient:Person {{case_id: $case_id, source_type: 'cellebrite'}})-[:PARTICIPATED_IN]->(chat)
                    WHERE msg.case_id = $case_id
                      ...
                      {app_filter_msg}{date_filter_msg}{att_filter_msg}
                """
                for rec in session.run(msg_match + """
                    WITH sender AS contact, msg
                    WHERE contact.key IS NOT NULL
                    RETURN contact.key AS key, contact.name AS name,
                           coalesce(contact.is_phone_owner, false) AS is_owner,
                           msg.source_app AS app, count(DISTINCT msg) AS n
                """, **params):
                    _apply(rec, "message_in")
                for rec in session.run(msg_match + """
                    WITH recipient AS contact, sender, msg
                    WHERE contact IS NOT NULL AND contact.key IS NOT NULL
                      AND contact.key <> sender.key
                    RETURN contact.key AS key, contact.name AS name,
                           coalesce(contact.is_phone_owner, false) AS is_owner,
                           msg.source_app AS app, count(DISTINCT msg) AS n
                """, **params):
                    _apply(rec, "message_out")

            if "call" in active_types:
                # Same strict src→call→dst join the envelope uses; emit one row
                # crediting the caller (inbound) and one crediting the callee
                # (outbound) so a single call feeds both counterparties' tallies.
                call_match = f"""
                    MATCH (c:PhoneCall {{case_id: $case_id, source_type: 'cellebrite'}})
                    WHERE coalesce(c.date, c.timestamp, '') <> ''
                      {rk_filter_call}{app_filter_call}{date_filter_call}{att_filter_call}
                    ...
                    WHERE true{from_filter_call}{to_filter_call}{inv_filter_call}
                """
                for rec in session.run(call_match + """
                    WITH src AS contact, c
                    WHERE contact.key IS NOT NULL
                    RETURN contact.key AS key, contact.name AS name,
                           coalesce(contact.is_phone_owner, false) AS is_owner,
                           c.source_app AS app, count(DISTINCT c) AS n
                """, **params):
                    _apply(rec, "call_in")
                for rec in session.run(call_match + """
                    WITH dst AS contact, c
                    WHERE contact.key IS NOT NULL
                    RETURN contact.key AS key, contact.name AS name,
                           coalesce(contact.is_phone_owner, false) AS is_owner,
                           c.source_app AS app, count(DISTINCT c) AS n
            ...
            if "email" in active_types:
                email_match = f"""
                    ...
                    WHERE true{from_filter_email}{to_filter_email}{inv_filter_email}
                """
                for rec in session.run(email_match + """
                    WITH a AS contact, e
                    WHERE contact.key IS NOT NULL
                    RETURN contact.key AS key, contact.name AS name,
                           coalesce(contact.is_phone_owner, false) AS is_owner,
                           e.source_app AS app, count(DISTINCT e) AS n
                """, **params):
                    _apply(rec, "email_in")
                for rec in session.run(email_match + """
                    WITH b AS contact, e
                    ...
                    RETURN contact.key AS key, contact.name AS name,
                           ...
                           e.source_app AS app, count(DISTINCT e) AS n
                """, **params):
                    _apply(rec, "email_out")

        # Resolve prettier per-identity names (same source the entity filter
        # uses) so a contact ranked here reads the way it does everywhere else.
        pa = self.person_aliases(case_id)

        def _finalize(r: dict) -> dict:
            total = (
                r["call_in"] + r["call_out"]
                + r["message_in"] + r["message_out"]
                + r["email_in"] + r["email_out"]
            )
            aliases = pa.get(r["key"], [])
            name = r["name"]
            if (not name or name == r["key"]) and aliases:
                name = aliases[0].get("name") or name
            return {**r, "name": name, "total": total}

        finalized = [_finalize(r) for r in acc.values()]

        owners = [r for r in finalized if r["is_owner"]]
        contacts = [r for r in finalized if not r["is_owner"]]
        contacts.sort(key=lambda r: (-r["total"], (r["name"] or "").lower()))
        owners.sort(key=lambda r: -r["total"])

        # Case-level totals frame the owner's inbound/outbound: sum across the
        # NON-owner contacts (owner rows excluded) so group-chat/self edges
        # don't double-count the subject against themselves.
        totals = {
            "message_in": 0, "message_out": 0,
            "call_in": 0, "call_out": 0,
            "email_in": 0, "email_out": 0,
        ...
        for r in contacts:
            for k in ("message_in", "message_out", "call_in", "call_out",
                      "email_in", "email_out", "total"):
                totals[k] += r[k]
            for app, n in r["by_platform"].items():
                totals["by_platform"][app] = totals["by_platform"].get(app, 0) + n

        contact_count = len(contacts)
        truncated = max(0, contact_count - rank_limit) if rank_limit else 0
        ranked = contacts[:rank_limit] if rank_limit else contacts
