...
def _commit_if_request_db(db: Session | None) -> None:
    if db is not None:
        db.commit()
...
class TriageService:
    ...
    def delete_case(self, case_id: str, db: Session | None = None) -> bool:
        ...
        deleted = triage_storage.delete_case(case_id, db=db)
        _commit_if_request_db(db)
        if deleted:
    ...
    def start_scan(self, case_id: str, resume: bool = False, db: Session | None = None) -> str:
        ...
        triage_storage.update_case(case_id, status="scanning", db=db)
    ...
    def start_classification(self, case_id: str, db: Session | None = None) -> str:
        ...
        # Update status
        triage_storage.update_case(case_id, status="classifying", db=db)
        triage_storage.update_stage(
            case_id,
            classify_stage["id"],
            db=db,
    ...
    def upload_hash_set(
        ...
        count = hash_lookup_service.add_custom_hash_set(
            name,
            hashes,
            created_by=created_by,
            db=db,
    ...
    def list_hash_sets(self, db: Session | None = None) -> List[Dict]:
        """List available custom hash sets."""
        return hash_lookup_service.list_custom_sets(db=db)

    # ── Profiling (Stage 2) ──────────────────────────────────────────

    def generate_profile(self, case_id: str, db: Session | None = None) -> str:
        ...
        triage_storage.update_case(case_id, status="profiling", db=db)
        triage_storage.update_stage(
            case_id,
            profile_stage["id"],
            db=db,
    ...
    def get_profile(self, case_id: str, db: Session | None = None) -> Optional[Dict]:
    ...
    def execute_stage(
        ...
        triage_storage.update_case(case_id, status="processing", db=db)
        triage_storage.update_stage(
            case_id, stage_id,
            db=db,
            status="running",
    ...
    def list_templates(self, db: Session | None = None) -> List[Dict]:
        ...
        return template_service.list_templates(db=db)

    def get_template(self, template_id: str, db: Session | None = None) -> Optional[Dict]:
        """Get a template by ID."""
        return template_service.get_template(template_id, db=db)
    ...
    def apply_template(
        ...
        stages = template_service.apply_template(template_id, case_id, db=db)
        _commit_if_request_db(db)
        return stages

    def delete_template(self, template_id: str, db: Session | None = None) -> bool:
        """Delete a template."""
        deleted = template_service.delete_template(template_id, db=db)
        _commit_if_request_db(db)
        return deleted
    ...
    def get_scan_stats(self, case_id: str, db: Session | None = None) -> Dict:
