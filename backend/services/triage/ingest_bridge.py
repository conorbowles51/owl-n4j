...
class IngestBridge:
    ...
    def ingest(
        ...
        for i in range(0, total, batch_size):
            ...
            if uploads:
                try:
                    records = self._register_evidence_files(
    ...
    def _register_evidence_files(
        ...
        *,
        ...
        """Copy triage source files into evidence storage and create Postgres records."""
        case_uuid = uuid.UUID(str(target_case_id))
        user_uuid = uuid.UUID(str(created_by_id)) if created_by_id else None
        case_dir = EVIDENCE_ROOT_DIR / str(target_case_id) / "triage" / triage_case_id
        ...
        with get_background_session() as db:
            for upload in uploads:
                ...
                relative_path = (upload.get("relative_path") or source_path.name).replace("\\", "/").lstrip("/")
                stored_path = case_dir / relative_path
                stored_path.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(source_path, stored_path)

                relative_parent = str(Path(relative_path).parent).replace("\\", "/")
                folder_relative_path = f"triage/{triage_case_id}"
                if relative_parent and relative_parent != ".":
                    folder_relative_path = f"{folder_relative_path}/{relative_parent}"
                ...
                records = EvidenceDBStorage.add_files(
                    db,
                    case_id=case_uuid,
                    files_data=[{
                        "original_filename": upload.get("original_filename") or source_path.name,
                        "stored_path": str(stored_path),
                        "sha256": sha256,
                        "size": upload.get("size") or stored_path.stat().st_size,
                    }],
                    owner=owner,
                    folder_id=folder_id,
                    created_by_id=user_uuid,
