...
def slide2(prs):
    ...
    para(tf3, "**And the tools that could hold it throw it away.** The document-review "
              "platforms say so in their own documentation: call logs export \u201conly as "
              "call log data in Excel format, not as individual records\u201d; multi-device "
              "extractions unsupported; conversations cut into fixed blocks.",
...
def slide3(prs):
    ...
    items = [
        ...
        ("03", "The category converged on cited answers — and stopped there.",
         "In four months the major review platforms folded AI into base pricing at no extra cost. Everyone can cite a document. Nobody modelled the case."),
    ]
    firstdone = False
...
def slide5(prs):
    ...
    para(tf, "**And not only ChatGPT — the specialists publish their own ceilings.** "
             "One caps at 300,000 documents per index; another states its search "
...
def slide5b(prs):
    ...
    cols = [
        ("They model the evidence — and won't sell it to you",
         ...
         "They own the extraction and build real graphs. The analytical tiers are sold "
         "to law enforcement and intelligence only; the leading unlocking hardware is "
         "not available to the private sector at all. What a practice can buy is "
         "examiner tooling — five kinds of device, no kinds of document."),
        ("They'll sell to you — and treat a phone as an attachment",
         "Document review & legal AI",
        ...
        ("Same buyer as us — and no case model",
         "Defence-native AI startups",
         "Four funded companies sell AI to criminal defence, $2.5M–$7.5M seed each, "
...
def slide6(prs):
    ...
    para(tf, "Our CPO is our design partner. Development has been customer-led "
             "from the first commit — a backlog generated by real federal "
             "casework. Cases have been worked to completion in the platform, and "
    ...
    para(tf, "Enterprise platforms are sold with configuration and support "
             "attached — the right model for agencies and large institutions, who "
             "arrive with budget and implementation resource.",
         size=12, colour=MUTED, space_before=7)
    para(tf, "The tier below has the same casework complexity and the same "
             "evidence volumes, and none of that budget. **Loupe is built for it "
             "directly: usable out of the box, by a practice with no "
    ...
    para(tf2, "Second market — corporate intelligence", size=12, colour=ACCENT,
         bold=True, space_before=11)
    para(tf2, "Investigative consultancies: fraud, corruption, asset tracing. Their "
              "process is scope, collect, corroborate, analyse, report — **and the "
              "last three stages are junior-analyst hours in Excel.** Second-best "
              "fit of the seventeen buyers we scored.",
         size=12, colour=MUTED, space_before=3)
    para(tf2, "The spend is already there", size=12, colour=ACCENT, bold=True,
         space_before=11)
    para(tf2, "Junior analysts bill **$300–$500/hour**; a six-week team of four "
              "bills **$150k–$250k**. Corporate investigation services: **$4.8B "
              "today, $9.7B by 2035.** Not new budget — hours already billed.",
         size=12, colour=MUTED, space_before=3)
    para(tf2, "How many, and how to reach them", size=12, colour=ACCENT, bold=True,
         space_before=11)
    para(tf2, "**48,000–125,000 evidence-heavy US criminal-defence matters a year**, "
              "across one to five thousand practices; the UK adds ~80,200 open Crown "
              "Court cases. Both buyers are organised — ACFE alone has ~95k members.",
         size=12, colour=MUTED, space_before=3)
