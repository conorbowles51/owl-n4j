...
export function groupStatementReadings(files: StatementFile[]) {
  ...
  return [...groups.values()].map((versions) => {
    ...
    const depth = (file: StatementFile) => {
      ...
      while (file.statement_parent_evidence_id && byId.has(file.statement_parent_evidence_id) && !seen.has(file.statement_parent_evidence_id)) {
    ...
    const current = [...(active.length ? active : versions)].sort((a, b) =>
      (b.created_at || "").localeCompare(a.created_at || "") || depth(b) - depth(a) || b.id.localeCompare(a.id)
    )[0]
...
export function useStatementRegister(
  ...
  const saved = imports.data && grouped ? {
    ...imports.data,
    files: grouped.flatMap((file) => {
      const ids = new Set(file.readingVersions.map((version) => version.id))
      const entries = imports.data.files.filter((item) => ids.has(item.evidence_file_id))
      if (!entries.length) return []
      const latest = entries.find((entry) => entry.evidence_file_id === file.id)
      return [{
        evidence_file_id: file.id,
        current_transactions: entries.reduce((sum, entry) => sum + entry.current_transactions, 0),
        incomplete_count: entries.reduce((sum, entry) => sum + entry.incomplete_count, 0),
        receipt_review_count: entries.reduce((sum, entry) => sum + entry.receipt_review_count, 0),
        wire_review_count: entries.reduce((sum, entry) => sum + entry.wire_review_count, 0),
        periods: entries.flatMap((entry) => entry.periods).filter((period) => period.source_status === "admitted"),
        history_periods: entries.flatMap((entry) => entry.periods).filter((period) => period.source_status !== "admitted").length,
  ...
  return { files: { ...files, data: grouped }, imports: { ...imports, data: saved } }
}
