...
it("uploads then explicitly requests local preparation and opens prepared rows",async()=>{
 ...
 const ready=mount();fireEvent.change(screen.getByLabelText("PDF document"),{target:{files:[new File(['pdf'],'bank.pdf',{type:'application/pdf'})]}});fireEvent.click(screen.getByRole("button",{name:"Prepare PDF for review",exact:true}));
