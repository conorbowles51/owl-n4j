...
export function StatementReconciliationSummary({
  ...
  const difference =
    !pending && calculation?.available && calculation.difference_minor != null
      ...
      : null
  const values = [
    ["Printed opening balance", calculation?.opening_minor],
    [
    ...
    ["Printed closing balance", calculation?.printed_closing_minor],
  ]
  return (
    <section
      aria-label={
        ...
        {pending
          ...
          : difference === null
            ...
            : difference === 0n
              ...
              : `${format((difference < 0n ? -difference : difference).toString())} ${difference < 0n ? "short of" : "above"} the printed closing balance.`}
      </p>
      {!compact && (
